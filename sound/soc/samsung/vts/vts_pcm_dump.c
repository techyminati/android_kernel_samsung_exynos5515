		...
		size_t count, loff_t *ppos)
{
	...
	do {
		...
		if (end > info->buffer.bytes) {
			vts_dev_err(dev, "pointer Error=%#zx file_pointer=%#zx end=%#zx buffer.bytes=%#zx\n",
		...
		if (!size) {
			if (file->f_flags & O_NONBLOCK) {
				vts_dev_err(dev, "%d size is 0", info->id);
				return -EAGAIN;
			}
	...
	if (vts_data->running) {
		...
		vts_dev_err(dev, "%s (%d) : Wrong VTS status RESET", __func__, info->id);
		info->file_pointer = 0;
		info->pointer = 0;
	}
	return size;
...
void vts_pcm_dump_reset(int id)
{
	...
	vts_dev_info(vts_pcm_dump_dev_vts, "%s by %d", __func__, id);
	info->file_pointer = 0;
	info->pointer = 0;
}
