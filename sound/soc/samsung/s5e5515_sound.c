/*
 *  Driver for Madera CODECs on s5e5515
 *
...
static int s5e5515_late_probe(struct snd_soc_card *card)
{
	struct madera_drvdata *drvdata = card->drvdata;
	struct snd_soc_pcm_runtime *rtd;
	struct snd_soc_dai *aif_dai;
...
static const struct snd_soc_dapm_widget s5e5515_supply_widgets[] = {
...
static int s5e5515_probe(struct snd_soc_card *card)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(s5e5515_supply_widgets); i++) {
		const struct snd_soc_dapm_widget *w;
		struct regulator *r;

		w = &s5e5515_supply_widgets[i];
		switch (w->id) {
...
static struct snd_soc_dai_link s5e5515_dai[100] = {
	{
		.name = "RDMA0",
		.stream_name = "RDMA0",
...
static const struct snd_kcontrol_new s5e5515_controls[] = {
...
static const struct snd_soc_dapm_widget s5e5515_widgets[] = {
...
static const struct snd_soc_dapm_route s5e5515_routes[] = {
...
static struct snd_soc_card s5e5515_madera = {
	.name = "S5e5515-Madera",
	.owner = THIS_MODULE,
	.dai_link = s5e5515_dai,
	.num_links = ARRAY_SIZE(s5e5515_dai),

	.probe = s5e5515_probe,
	.late_probe = s5e5515_late_probe,

	.controls = s5e5515_controls,
	.num_controls = ARRAY_SIZE(s5e5515_controls),
	.dapm_widgets = s5e5515_widgets,
	.num_dapm_widgets = ARRAY_SIZE(s5e5515_widgets),
	.dapm_routes = s5e5515_routes,
	.num_dapm_routes = ARRAY_SIZE(s5e5515_routes),
		...
		struct snd_soc_dai_link *dai_link)
{
	...
	if (dai_link->cpu_dai_name == NULL) {
		/* Ignoring the return as we don't register DAIs to the
		 * platform */
...
static void s5e5515_register_card_work_func(struct work_struct *work)
{
	struct snd_soc_card *card = &s5e5515_madera;
	int ret;
...
DECLARE_WORK(s5e5515_register_card_work, s5e5515_register_card_work_func);

static int s5e5515_audio_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &s5e5515_madera;
	struct madera_drvdata *drvdata = card->drvdata;
	...
	schedule_work(&s5e5515_register_card_work);

	return ret;
}

static int s5e5515_audio_remove(struct platform_device *pdev)
{
...
static const struct of_device_id s5e5515_audio_of_match[] = {
	{ .compatible = "samsung,s5e5515-madera", },
	{},
};
MODULE_DEVICE_TABLE(of, s5e5515_audio_of_match);
#endif /* CONFIG_OF */

static struct platform_driver s5e5515_audio_driver = {
	.driver		= {
		.name	= "s5e5515-audio",
		.owner	= THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = of_match_ptr(s5e5515_audio_of_match),
	},

	.probe		= s5e5515_audio_probe,
	.remove		= s5e5515_audio_remove,
};

module_platform_driver(s5e5515_audio_driver);

MODULE_DESCRIPTION("ALSA SoC S5e5515 Audio Driver");
MODULE_AUTHOR("Charles Keepax <ckeepax@opensource.wolfsonmicro.com>");
MODULE_AUTHOR("Gyeongtaek Lee <gt82.lee@samsung.com>");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:s5e5515-madera");
