...
static int abox_dapm_set_route_weak(struct snd_soc_dapm_context *dapm,
				   const struct snd_soc_dapm_route *route, int weak)
{
	struct snd_soc_dapm_widget *wsource = NULL;
	struct snd_soc_dapm_widget *wsink = NULL;
	...
	if (!dapm->component) {
		abox_warn(dapm->dev, "Component of dapm is not found\n");
		return 0;
	}
	...
	if (!wsource) {
		abox_err(dapm->dev, "Unable to find source %s for abox weak route\n", source);
		return -ENODEV;
	}

	list_for_each_entry(w, &dapm->card->widgets, list) {
		if (!strcmp(w->name, sink)) {
			wsink = w;
			break;
		}
	}
	...
	if (!wsink) {
		abox_err(dapm->dev, "Unable to find sink %s for abox weak route\n", sink);
		return -ENODEV;
	}

	snd_soc_dapm_widget_for_each_sink_path(wsource, path) {
		if (path->sink == wsink) {
			path->weak = weak;
			count++;
		}
	...
	if (count == 0)
		abox_err(dapm->dev, "No path found for weak route %s->%s\n",
			source, sink);
	if (count > 1)
		abox_dbg(dapm->dev, "(%d) paths found for weak route %s->%s\n",
			 count, source, sink);

	return 0;
}

static const struct snd_soc_dapm_route weak_routes[] = {
	/* sink, control, source */
	{"VSS_TXADAPTER", NULL, "VSS_TXADAPTER_A"},
};

static int weak_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = data->weak_route.weak;

	return 0;
}

static int weak_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct device *dev = cmpnt->dev;
	...
	data->weak_route.weak = !!val;

	for (i = 0; i < ARRAY_SIZE(weak_routes); i++) {
		abox_info(dev, "[%s] source: %s -> sink: %s\n",
				!!val ? "ADD" : "DEL", weak_routes[i].source,
				weak_routes[i].sink);
		abox_dapm_set_route_weak(dapm, &weak_routes[i], !!val);
	}
		...
		unsigned int channels, enum ABOX_CONFIGMSG configmsg)
{
	...
	case SET_SIFS0_FORMAT:
		...
		regmap_update_bits(data->regmap, ABOX_SPUS_CTRL1,
				...
				ABOX_MIXP_FORMAT_L);
		/* Set STMIX format */
		regmap_update_bits(data->regmap, ABOX_SPUS_CTRL_MIXP_FORMAT,
...
static const struct snd_kcontrol_new cmpnt_controls[] = {
	...
	SOC_SINGLE_EXT("Weak", 0, 0, 1, 0, weak_get, weak_put),
...
static enum abox_dai get_sink_dai_id(struct abox_data *data, enum abox_dai id)
{
	...
	case ABOX_UAIF6:
	case ABOX_SPDY:
		for (_id = ABOX_NSRC0; _id <= ABOX_NSRC4; _id++) {
		...
		unsigned int width, int apf_coef)
{
	...
	abox_info(dev, "%s(%d, %d, %uHz, %uHz, %d, %ubit, %d)\n", __func__,
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(nsrc0_enum,
		ABOX_ROUTE_CTRL1, ABOX_ROUTE_NSRC_L(0),
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(nsrc1_enum,
		ABOX_ROUTE_CTRL1, ABOX_ROUTE_NSRC_L(1),
		nsrcx_mask, nsrcx_texts, nsrcx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(nsrc2_enum,
		ABOX_ROUTE_CTRL1, ABOX_ROUTE_NSRC_L(2),
		nsrcx_mask, nsrcx_texts, nsrcx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(nsrc3_enum,
		ABOX_ROUTE_CTRL1, ABOX_ROUTE_NSRC_L(3),
		nsrcx_mask, nsrcx_texts, nsrcx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(nsrc4_enum,
		ABOX_ROUTE_CTRL1, ABOX_ROUTE_NSRC_L(4),
		nsrcx_mask, nsrcx_texts, nsrcx_values);
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (ret < 0) {
		abox_info(dev, "%s - fail sbank set %d\n", __func__, ret);
		return ret;
	}
