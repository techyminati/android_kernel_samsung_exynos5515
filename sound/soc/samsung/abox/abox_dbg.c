...
#define MIN_ABOX_SLOG_RMEM_SZ (0x200000) /* 2Mb */
static void abox_slog_free_reserved_mem(void)
{
	pr_info("%s: before base=%pa, size=%pa\n", __func__,
			&abox_dbg_slog->base, &abox_dbg_slog->size);

	free_reserved_area((void *)(phys_to_virt(abox_dbg_slog->base) + MIN_ABOX_SLOG_RMEM_SZ),
				(void *)(phys_to_virt(abox_dbg_slog->base) + abox_dbg_slog->size), 0, "abox_slog");

	abox_dbg_slog->size = MIN_ABOX_SLOG_RMEM_SZ;

	pr_info("%s: after base=%pa, size=%pa\n", __func__, &abox_dbg_slog->base, &abox_dbg_slog->size);
}
#endif
...
#ifdef CONFIG_SND_SOC_SAMSUNG_ABOX_FREE_RMEM
	if(abox_get_debug_level() == 0) {
		abox_slog_free_reserved_mem();
	}
#endif
...
static void abox_dbg_alloc_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < ABOX_DBG_DUMP_COUNT; i++) {
		...
		if(abox_test_quirk(data, ABOX_QUIRK_BIT_WEARABLE_MODE))
			continue;
...
*/
	...
	if (p_abox_dbg_dump_min) {
		abox_info(dev, "%s - p_abox_dbg_dump_min\n", __func__);
