...
static ssize_t set_buffer_alloc_type(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
        int ret;
        struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
        ...
        ret = kstrtoint(buf, 0, &platform->buffer_alloc_type);
        if (ret) {
                GPU_LOG(DVFS_WARNING, DUMMY, 0u, 0u, "%s: invalid value\n", __func__);
                return -ENOENT;
        }

        GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "%s: buffer_alloc_type %d\n", __func__, platform->buffer_alloc_type);

        return count;
}


static ssize_t show_buffer_alloc_type(struct device *dev, struct device_attribute *attr, char *buf)
{
        ssize_t ret = 0;
        struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
        ...
        ret += snprintf(buf+ret, PAGE_SIZE-ret, "%d", platform->buffer_alloc_type);
...
#endif
#ifdef CONFIG_MALI_BUFFER_ALLOC_TYPE
DEVICE_ATTR(buffer_alloc_type, S_IRUGO|S_IWUSR, show_buffer_alloc_type, set_buffer_alloc_type);
#endif
#ifdef CONFIG_MALI_SEC_CL_BOOST
...
#endif
#ifdef CONFIG_MALI_BUFFER_ALLOC_TYPE
	if (device_create_file(dev, &dev_attr_buffer_alloc_type)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "couldn't create sysfs file [buffer_alloc_type]\n");
		goto out;
	}
#endif
#ifdef CONFIG_MALI_TSG
...
#endif
#ifdef CONFIG_MALI_BUFFER_ALLOC_TYPE
	device_remove_file(dev, &dev_attr_buffer_alloc_type);
#endif
#ifdef CONFIG_MALI_TSG
