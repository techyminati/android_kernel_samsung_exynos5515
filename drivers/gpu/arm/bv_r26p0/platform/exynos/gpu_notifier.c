...
static bool skip_get_sync = false;
...
static int gpu_power_on(struct kbase_device *kbdev)
{
	...
	/* Block Runtime PM API during resuming if gpu is already powered on */
	if (platform->is_resuming)
	{
		/* GPU is powered off */
		if(!gpu_is_power_on())
		{
...
static void gpu_power_off(struct kbase_device *kbdev)
{
	...
	/* Block Runtime PM API during resuming */
	if (!skip_get_sync)
	{
