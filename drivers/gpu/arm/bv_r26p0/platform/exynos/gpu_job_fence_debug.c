...
int gpu_job_fence_status_dump(struct sync_file *guilty_fence)
#else
...
#endif
{
    ...
    if (guilty_fence != NULL) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0))
        dev_warn(dev, "Timeout Sync_file [%p] Sync_file name %s\n", guilty_fence, guilty_fence->name);
        dev_warn(dev, "Timeout Fence *** [%p] \n", guilty_fence->fence);
#else
		dev_warn(dev, "Timeout Fence *** [%p] fence name %s\n", guilty_fence, guilty_fence->ops->get_driver_name(guilty_fence));
#endif
    }
...
#else
                        dev_warn(dev, "  **Trigger Fence_out Info [%p] %s: fence type = 0x%x, fence ctx = %llu, fence seqno = %u\n",
                                info_out.fence, fence_out->ops->get_driver_name(fence_out), kctx->jctx.atoms[i].core_req, fence_out->context, fence_out->seqno);
#endif
					}
				}
...
#else
                        dev_warn(dev, "  **Wait Fence_in Info [%p] %s: fence type = 0x%x, fence ctx = %llu, fence seqno = %u\n",
                                info_in.fence, fence_in->ops->get_driver_name(fence_in), kctx->jctx.atoms[i].core_req, fence_in->context, fence_in->seqno);
#endif
					}
				}
				spin_unlock_irqrestore(&kctx->waiting_soft_jobs_lock, lflags);
