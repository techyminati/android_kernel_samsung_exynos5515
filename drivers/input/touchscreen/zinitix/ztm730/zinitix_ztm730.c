...
#if ZTM_MOTOR_DEV
#include <linux/ztm730_motor.h>
#endif
#ifdef ZINITIX_MISC_DEBUG
struct ztm730_info *misc_info;
#endif
...
static bool ztm730_regulator_on(struct ztm730_info *info);
static bool ztm730_regulator_off(struct ztm730_info *info);
static bool ztm730_power_sequence(struct ztm730_info *info);
static bool ztm730_init_touch(struct ztm730_info *info, bool forced);
static bool ztm730_mini_init_touch(struct ztm730_info *info);
static void ztm730_clear_report_data(struct ztm730_info *info);
#ifdef CONFIG_OF
static int ztm730_pinctrl_configure(struct ztm730_info *info);
#endif

static bool ztm730_hw_calibration(struct ztm730_info *info);
static s32 ztm730_write_reg(struct i2c_client *client, u16 reg, u16 value);
static s32 ztm730_write_cmd(struct i2c_client *client, u16 reg);
static s32 ztm730_read_data(struct i2c_client *client, u16 reg, u8 *values, u16 length);

#ifdef SEC_FACTORY_TEST
static bool ztm730_set_touchmode(struct ztm730_info *info, u16 value);
static int ztm730_upgrade_sequence(struct ztm730_info *info, const u8 *firmware_data);
...
#ifdef CONFIG_SLEEP_MONITOR
static int ztm730_get_sleep_monitor_cb(void* priv, unsigned int *raw_val, int check_level, int caller_type);
static struct sleep_monitor_ops  ztm730_sleep_monitor_ops = {
	.read_cb_func =  ztm730_get_sleep_monitor_cb,
};

static int ztm730_get_sleep_monitor_cb(void* priv, unsigned int *raw_val, int check_level, int caller_type)
{
	struct ztm730_info *info = priv;
...
#if defined(CYTEST_GETPOWERMODE)
		tsp_mode = ztm730_test_get_power_mode(info->dev);
		if (tsp_mode < 0) {
...
static void ztm730_delay(unsigned int ms)
{
	if (ms <= 20)
...
#if TOUCH_POINT_MODE
static s32 ztm730_read_data_only(struct i2c_client *client, u8 *values, u16 length)
{
	struct ztm730_info *info = i2c_get_clientdata(client);
	struct i2c_adapter *adapter = client->adapter;
	struct dev_pm_info *i2c_dev_pm = &adapter->dev.power;
...
static inline s32 ztm730_write_nvm_data(struct i2c_client *client)
{
	struct ztm730_info *info = i2c_get_clientdata(client);
	struct i2c_adapter *adapter = client->adapter;
	struct dev_pm_info *i2c_dev_pm = &adapter->dev.power;
	...
	if (ret < 0) {
		info->comm_err_cnt++;
		dev_err(&client->dev, "%s:failed to send(%d)\n",
					__func__, ret);
		return ret;
	}
...
static inline s32 ztm730_write_data(struct i2c_client *client, u16 reg, u8 *values, u16 length)
{
	struct ztm730_info *info = i2c_get_clientdata(client);
	struct i2c_adapter *adapter = client->adapter;
	struct dev_pm_info *i2c_dev_pm = &adapter->dev.power;
...
static inline s32 ztm730_read_raw_data(struct i2c_client *client, u16 reg, u8 *values, u16 length)
{
	struct ztm730_info *info = i2c_get_clientdata(client);
	struct i2c_adapter *adapter = client->adapter;
	struct dev_pm_info *i2c_dev_pm = &adapter->dev.power;
...
static int ztm730_read_finger_cnt(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	s32 ret, val = 0;

	ret = ztm730_read_data(info->client, ZTM730_FINGER_CNT_REG, (u8 *)&val, 2);
	if (ret < 0) {
		dev_err(&client->dev,
			"%s:Failed to read ZTM730_FINGER_CNT_REG register\n", __func__);
		return -1;
	}
...
static int  ztm730_send_deep_sleep_cmd(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
...
static int  ztm730_write_wakeup_cmd(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret, i;
...
static int ztm730_set_optional_mode(struct ztm730_info *info, bool force)
{
	...
	if (ret) {
		ret = ztm730_write_reg(info->client, ZTM730_WHEEL_VIB_SETTING, info->vib_mode);
		if (ret != I2C_SUCCESS) {
			dev_err(&info->client->dev,
...
static void ztm730_chip_reset(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
	...
	if (ret) {
		dev_err(&info->client->dev,
			"%s:Failed to ztm730_regulator_off\n", __func__);
	}
	...
	if (ret) {
		dev_err(&info->client->dev,
			"%s:Failed to ztm730_regulator_on\n", __func__);
	}
...
static void ztm730_set_ta_status(struct ztm730_info *info)
{
	int ret;
...
static void ztm730_ts_charger_status_cb(struct tsp_callbacks *cb, int status)
{
...
static void ztm730_register_callback(struct tsp_callbacks *cb)
{
...
static int read_fw_verify_result(struct ztm730_info *info);
static void ztm730_get_dqa_data(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
...
static void ztm730_store_raw_data(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
#ifdef SEC_FACTORY_TEST
...
static bool ztm730_get_raw_data(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	u32 total_node = info->cap_info.total_node_num;
	int sz;
...
static bool ztm730_clear_interrupt(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
...
static bool ztm730_ts_read_coord(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
#if TOUCH_POINT_MODE
	...
	if (info->touch_info.event_flag == 0 || info->touch_info.status == 0) {
		...
		ret = ztm730_write_cmd(info->client, ZTM730_CLEAR_INT_STATUS_CMD);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:failed to write ZTM730_CLEAR_INT_STATUS_CMD\n", __func__);
		}
		return true;
	}
...
static void set_gesture_mode(struct ztm730_info *info, bool enable) 
{
...
static int ztm730_soft_reset(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
...
static int ztm730_gpio_reset(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	struct zxt_ts_platform_data	*pdata = info->pdata;
	int ret = 0;
...
static int ztm730_mode_change_reset(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	struct zxt_ts_platform_data	*pdata = info->pdata;
	int ret = 0;
	...
	ret = ztm730_power_sequence(info);
	if (!ret) {
		dev_err(&client->dev, "%s: Failed to ztm730_power_sequence\n", __func__);
		ret = -ENXIO;
		goto out;
	}
...
static int ztm730_hard_reset(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret = 0;
	...
	if (ret) {
		dev_err(&client->dev, "%s:Failed to ztm730_gpio_reset\n", __func__);
		goto err_out;
	}
	...
	enable_irq(info->irq);
#if ZTM_MOTOR_DEV
	ret = ztm730_motor_reset_handler();
	if (ret) {
		dev_err(&client->dev,
			"%s:ztm730_motor reset handler failed. [%d]\n",
			__func__, ret);
	}
#endif
...
static bool ztm730_check_need_upgrade(struct ztm730_info *info,
...
static bool ztm730_upgrade_firmware(struct ztm730_info *info, const u8 *firmware_data, u32 size)
{
	struct i2c_client *client = info->client;
	u32 flash_addr;
	u8 *verify_data;
...
///add nvm timing
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to set osc sel\n", __func__);
		goto fail_upgrade;
	}
	ztm730_delay(5);
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to set nvm timing\n", __func__);
		goto fail_upgrade;
	}
	ztm730_delay(5);//5ms
...
static bool ztm730_hw_calibration(struct ztm730_info *info)
{
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM730_TOUCH_MODE\n", __func__);
		return false;
	}
	ztm730_delay(10);
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM730_CALIBRATE_CMD\n", __func__);
		return false;
	}
	usleep_range(DELAY_FOR_POST_TRANSCATION, DELAY_FOR_POST_TRANSCATION);
	...
	do {
		...
		if (ret < 0) {
			dev_err(&client->dev, "%s:failed to write ZTM730_EEPROM_INFO_REG\n", __func__);
			return false;
		}
	...
	if (info->cap_info.ic_int_mask) {
		...
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:failed to write ZTM730_INT_ENABLE_FLAG\n", __func__);
			return false;
		}
	}
	usleep_range(100, 100);
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM730_SAVE_CALIBRATION_CMD\n", __func__);
		return false;
	}
	ztm730_delay(1000);
...
static int ztm730_get_ic_fw_size(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	struct capa_info *cap = &(info->cap_info);
	int ret = 0;

	if (info->chip_code == ZTM730_IC_CHIP_CODE || info->chip_code == ZTM730_IC_CHIP_CODE)
		cap->ic_fw_size = ZTM730_IC_FW_SIZE;
	else if(info->chip_code == ZTM730_IC_CHIP_CODE)	
		cap->ic_fw_size = ZTM730_IC_FW_SIZE;
	else {
...
static bool ztm730_init_touch(struct ztm730_info *info, bool fw_skip)
{
...
retry_init:
	...
	dev_info(&client->dev, "%s:ZTM730_EEPROM_INFO_REG:[0x%x]\n", __func__, chip_eeprom_info);
	...
	checksum_err = false;
	for (i = 0; i < INIT_RETRY_CNT; i++) {
		ret = ztm730_read_data(client, ZTM730_CHECKSUM_RESULT,
						(u8 *)&chip_check_sum, 2);
		if (ret < 0) {
			dev_err(&client->dev, "%s:Failed to read ZTM730_CHECKSUM_RESULT[0x%04x]\n",
				__func__, chip_check_sum);
			ztm730_delay(10);
			continue;
		}
		dev_info(&client->dev, "%s:ZTM730_CHECKSUM_RESULT:[0x%x]\n", __func__, chip_check_sum);
	...
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read ZTM730_HW_ID\n", __func__);
		goto fail_init;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_FIRMWARE_VERSION[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_MINOR_FW_VERSION, (u8 *)&cap->fw_minor_version, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_MINOR_FW_VERSION[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_DATA_VERSION_REG, (u8 *)&cap->reg_data_version, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_DATA_VERSION_REG[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_EEPROM_INFO_REG, (u8 *)&chip_eeprom_info, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_EEPROM_INFO_REG[%d]\n", __func__, ret);
		goto fail_init;
	}


	ret = ztm730_check_need_upgrade(info, cap->fw_version,
	...
	if (ret && !fw_skip) {
		...
		if (!ret) {
			dev_err(&client->dev, "%s:Failed ztm730_upgrade_firmware[%d]\n", __func__, ret);
			goto fail_init;
		}
	...
	ret = ztm730_read_data(client, ZTM730_THRESHOLD, (u8 *)&cap->threshold, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_THRESHOLD[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_BUTTON_SENSITIVITY, (u8 *)&cap->key_threshold, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_BUTTON_SENSITIVITY[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_DUMMY_BUTTON_SENSITIVITY, (u8 *)&cap->dummy_threshold, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_DUMMY_BUTTON_SENSITIVITY[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_TOTAL_NUMBER_OF_X, (u8 *)&cap->x_node_num, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_TOTAL_NUMBER_OF_X[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_TOTAL_NUMBER_OF_Y, (u8 *)&cap->y_node_num, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_TOTAL_NUMBER_OF_Y[%d]\n", __func__, ret);
		goto fail_init;
	}
	...
	ret = ztm730_read_data(client, ZTM730_DND_CP_CTRL_L, (u8 *)&cap->cp_ctrl_l, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_DND_CP_CTRL_L[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_DND_V_FORCE, (u8 *)&cap->v_force, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_DND_V_FORCE[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_DND_AMP_V_SEL, (u8 *)&cap->amp_v_sel, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_DND_AMP_V_SEL[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_DND_N_COUNT, (u8 *)&cap->N_cnt, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_DND_N_COUNT[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_DND_U_COUNT, (u8 *)&cap->u_cnt, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_DND_U_COUNT[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_read_data(client, ZTM730_AFE_FREQUENCY, (u8 *)&cap->afe_frequency, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM730_AFE_FREQUENCY[%d]\n", __func__, ret);
		goto fail_init;
	}
	...
	if (ret) {
		...
		ret = ztm730_write_reg(client, ZTM730_INT_ENABLE_FLAG, 0);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:Failed ZTM730_INT_ENABLE_FLAG[%d]\n", __func__, ret);
			goto fail_init;
		}
	}
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM730_X_RESOLUTION[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_write_reg(client, ZTM730_Y_RESOLUTION, (u16)pdata->y_resolution + ABS_PT_OFFSET);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM730_Y_RESOLUTION[%d]\n", __func__, ret);
		goto fail_init;
	}

	if (pdata->orientation != -1) {
		ret = ztm730_write_reg(client, ZTM730_COORD_ORIENTATION, (u16)pdata->orientation);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:Failed ZTM730_COORD_ORIENTATION[%d]\n", __func__, ret);
			goto fail_init;
		}
	}
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM730_SUPPORTED_FINGER_NUM[%d]\n", __func__, ret);
		goto fail_init;
	}
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM730_INITIAL_TOUCH_MODE[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm730_write_reg(client, ZTM730_TOUCH_MODE, info->touch_mode);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM730_TOUCH_MODE[%d]\n", __func__, ret);
		goto fail_init;
	}
	...
	for (i = 0; i < 10; i++) {
		ret = ztm730_write_cmd(client, ZTM730_CLEAR_INT_STATUS_CMD);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:failed to write ZTM730_CLEAR_INT_STATUS_CMD[%d]\n", __func__, i);
		}
		usleep_range(10, 10);
	}
...
fail_init:
	...
	if (++retry_cnt <= INIT_RETRY_CNT) {
		...
		if (!chip_code) {
			info->chip_code = ZTM730_IC_CHIP_CODE;
			ret = ztm730_get_ic_fw_size(info);
			if (ret) {
				dev_err(&client->dev, "%s:Failed to get ic_fw_size\n", __func__);
				goto retry_fail_init;
...
#if ZTM_MOTOR_DEV
extern bool ztm730_motor_is_running(void);
#endif
static int ztm730_mode_change_recovery(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;

#if ZTM_MOTOR_DEV
	ret = ztm730_motor_is_running();
	if (ret) {
		dev_info(&client->dev, "%s:Skip mode_change_recovery by motor\n", __func__);
		return 0;
	}
#endif
	ret = ztm730_mode_change_reset(info);
	if (ret) {
		dev_err(&client->dev, "%s:Failed to hard reset\n", __func__);
		return -1;
	}
...
static int ztm730_hard_reset_recovery(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
	...
	ret = ztm730_init_touch(info, false);
	if (!ret) {
		dev_err(&client->dev, "%s:Failed to initialize\n", __func__);
		return -1;
	}
...
static bool ztm730_mini_init_touch(struct ztm730_info *info)
{
	...
	ret = ztm730_write_reg(client, ZTM730_X_RESOLUTION, (u16)(pdata->x_resolution + ABS_PT_OFFSET));
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM730_X_RESOLUTION\n", __func__);
		goto fail_mini_init;
	}

	ret = ztm730_write_reg(client, ZTM730_Y_RESOLUTION, (u16)(pdata->y_resolution + ABS_PT_OFFSET));
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM730_Y_RESOLUTION\n", __func__);
		goto fail_mini_init;
	}
	...
	if (pdata->orientation != -1) {
		ret = ztm730_write_reg(client, ZTM730_COORD_ORIENTATION, (u16)pdata->orientation);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:failed to write ZTM730_COORD_ORIENTATION\n", __func__);
			goto fail_mini_init;
		}
	}
	...
	ret = ztm730_write_reg(client, ZTM730_SUPPORTED_FINGER_NUM, (u16)MAX_SUPPORTED_FINGER_NUM);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM730_SUPPORTED_FINGER_NUM\n", __func__);
		goto fail_mini_init;
	}

	ret = ztm730_write_reg(client, ZTM730_INITIAL_TOUCH_MODE, TOUCH_POINT_MODE);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM730_INITIAL_TOUCH_MODE\n", __func__);
		goto fail_mini_init;
	}
	...
	if (info->touch_mode != TOUCH_POINT_MODE) {
		ret = ztm730_write_reg(client, ZTM730_DELAY_RAW_FOR_HOST, RAWDATA_DELAY_FOR_HOST);
		if ( ret  != I2C_SUCCESS){
			dev_err(&client->dev, "%s:Failed to set ZTM730_DELAY_RAW_FOR_HOST\n", __func__);
			goto fail_mini_init;
		}
	}
...
fail_mini_init:
	...
	ret = ztm730_hard_reset_recovery(info);
	if (ret) {
		dev_err(&client->dev, "%s:Failed ztm730_hard_reset_recovery\n", __func__);
		return false;
	}
...
static void  ztm730_send_wakeup_event(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
...
static irqreturn_t ztm730_touch_work(int irq, void *data)
{
...
#endif
#ifdef CONFIG_PM
static int ztm730_resume(struct device *dev)
{
...
static int ztm730_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct ztm730_info *info = i2c_get_clientdata(client);
...
static int ztm730_store_gesture_on(struct ztm730_info *info);
static int ztm730_store_gesture_off(struct ztm730_info *info);

static int ztm730_input_open(struct input_dev *dev)
{
...
static void ztm730_input_close(struct input_dev *dev)
{
	struct ztm730_info *info = input_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
	...
	ret = ztm730_read_finger_cnt(info);
	if (ret < 0) {
		dev_err(&client->dev,
			"%s:Failed ztm730_read_finger_cnt ret:[%d] kernel:[%d]\n", __func__, ret, info->finger_event_cnt);
	} else
...
static void run_connect_test(void *device_data)
{
	struct ztm730_info *info = (struct ztm730_info *)device_data;
	struct i2c_client *client = info->client;
	struct tsp_factory_info *finfo = info->factory_info;
	u16 threshold = 0;
	int ret;
	...
	disable_irq(info->irq);
	ret = ztm730_read_data(client, ZTM730_CONNECTED_REG, (char *)&threshold, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read ZTM730_CONNECTED_REG\n", __func__);
		finfo->cmd_state = FAIL;
...
*/
	...
	case BUILT_IN:
		...
		ret = ztm730_upgrade_sequence(info, (u8 *)info->fw_data);
		if (ret < 0) {
			finfo->cmd_state = FAIL;
			return;
		}
	...
	case UMS:
		...
		ret = ztm730_upgrade_sequence(info, (u8 *)buff);
		if (ret < 0) {
			devm_kfree(&client->dev, buff);
			finfo->cmd_state = FAIL;
			goto update_fail;
...
#ifdef ZINITIX_FILE_DEBUG
static bool ztm730_touchmode_change(struct ztm730_info *info, u16 value)
{
	disable_irq(info->irq);
	down(&info->work_lock);
	if (info->work_state != NOTHING) {
	...
	dev_info(&info->client->dev, "%s:touchkey_testmode = %d\n",
			__func__, info->touch_mode);
	if (ztm730_write_reg(info->client, ZTM730_TOUCH_MODE,
...
static void set_touchmode(void *device_data)
{
	...
	if (ztm730_touchmode_change(info, finfo->cmd_param[0]) == true) {
		finfo->cmd_state = OK;
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", "OK");
	} else {
		finfo->cmd_state = FAIL;
...
static void set_touchreg(void *device_data)
{
	...
	ret = ztm730_write_reg(info->client, reg, val);
	if ( ret == I2C_SUCCESS) {
...
static void get_fw_verify_result(void *device_data)
{
	...
	dev_info(&info->client->dev,
		"%s:ZTM730_FW_CHECKSUM_VAL[0x%04x]\n", __func__, info->checksum);
...
static void get_touchreg(void *device_data)
{
	...
	reg = finfo->cmd_param[0] | (finfo->cmd_param[1]<<8);
	if (ztm730_read_data(info->client, reg, (u8 *)&val, 2) == 2) {
		dev_info(&info->client->dev, "%s:reg = %d(%04x) val = %d(%04x)\n", __func__, reg, reg, val, val);
		finfo->cmd_state = OK;
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s:reg = %d(%04x) val = %d(%04x)\n", __func__, reg, reg, val, val);
	} else {
...
static int read_fw_ver_bin(struct ztm730_info *info, u32 *version)
{
	const struct firmware *tsp_fw = NULL;
...
static void disable_factory_test(void *device_data)
{
	...
	ret = ztm730_write_reg(client, ZTM730_I2C_MODE_REG, REG_SET_I2C_LPM);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s:Failed to read ZTM730_I2C_MODE_REG. \n", __func__);
...
static void enable_factory_test(void *device_data)
{
	...
	ret = ztm730_write_reg(client, ZTM730_I2C_MODE_REG, REG_SET_I2C_NORMAL);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s:Failed to read ZTM730_I2C_MODE_REG. retry:[%d]\n", __func__, ret);
...
static int read_fw_ver_ic(struct ztm730_info *info, u32 *version)
{
	struct i2c_client *client = info->client;
	...
	ret = ztm730_read_data(client, ZTM730_CHIP_REVISION, (u8 *)&ic_name, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read ic_name\n", __func__);
		return -1;
	}

	ret = ztm730_read_data(client, ZTM730_FIRMWARE_VERSION, (u8 *)&major_ver, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read major_ver\n", __func__);
		return -1;
	}

	ret = ztm730_read_data(client, ZTM730_MINOR_FW_VERSION, (u8 *)&minor_ver, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read minor_ver\n", __func__);
		return -1;
	}

	ret = ztm730_read_data(client, ZTM730_HW_ID, (u8 *)&module_ver, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read model_version\n", __func__);
		return -1;
	}

	ret = ztm730_read_data(client, ZTM730_DATA_VERSION_REG, (u8 *)&fw_version, 2);
	if (ret < 0) {
...
static void get_chip_vendor(void *device_data)
{
	...
	snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZTM730_VENDOR_NAME);
	set_cmd_result(finfo, finfo->cmd_buff,
			strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
...
static void get_chip_name(void *device_data)
{
	...
	if (info->chip_code == ZTM730_IC_CHIP_CODE)
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZTM730_CHIP_NAME);
	else if (info->chip_code == ZTM730_IC_CHIP_CODE)
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZTM730_CHIP_NAME);
	else if (info->chip_code == ZTM730_IC_CHIP_CODE)
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s", ZTM730_CHIP_NAME);
...
static void get_all_data(struct ztm730_info *info,
...
static bool get_raw_data(struct ztm730_info *info, u8 *buff, int skip_cnt)
{
	struct i2c_client *client = info->client;
	struct zxt_ts_platform_data *pdata = info->pdata;
	u32 total_node = info->cap_info.total_node_num;
	...
	for (i = 0; sz > 0; i++) {
		...
		if (sz < I2C_BUFFER_SIZE)
			temp_sz = sz;
		if (ztm730_read_raw_data(client, ZTM730_RAWDATA_REG + i,
...
static void run_ssr_read(void *device_data)
{
	...
	ret = ztm730_set_touchmode(info, TOUCH_SSR_MODE);
	if (ret) {
...
out:
	...
	ret = ztm730_set_touchmode(info, TOUCH_POINT_MODE);
	if (ret)
...
static void run_sfr_read(void *device_data)
{
	...
	ret = ztm730_set_touchmode(info, TOUCH_SFR_MODE);
	if (ret) {
...
static void run_trx_short_read(void *device_data)
{
	...
	ret = ztm730_set_touchmode(info, TOUCH_TRXSHORT_MODE);
	if (ret) {
...
static void run_wheel_read(void *device_data)
{
	...
	ret = ztm730_set_touchmode(info, TOUCH_WHEEL_MODE);
	if (ret) {
...
static void run_dnd_read(void *device_data)
{
	...
	ret = ztm730_set_touchmode(info, TOUCH_DND_MODE);
	if (ret) {
		dev_err(&client->dev, "%s:Failed to set DND_MODE\n", __func__);
		finfo->cmd_state = FAIL;
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "NG");
...
static void run_hfdnd_read(void *device_data)
{
	...
	ret = ztm730_set_touchmode(info, TOUCH_HFDND_MODE);
	if (ret) {
...
static void run_delta_read(void *device_data)
{
	...
	ret = ztm730_set_touchmode(info, TOUCH_DELTA_MODE);
	if (ret) {
...
static void ztm730_cover_set(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
...
static void run_reference_read(void *device_data)
{
	...
	ret = ztm730_set_touchmode(info, TOUCH_REFERENCE_MODE);
	if (ret) {
...
static void clear_reference_data(void *device_data)
{
	...
	ret = ztm730_write_reg(client, ZTM730_EEPROM_INFO_REG, 0xffff);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM730_EEPROM_INFO_REG\n", __func__);
	}
	...
	ret = ztm730_write_cmd(client, ZTM730_SAVE_STATUS_CMD);
	if (ret != I2C_SUCCESS) {
...
static void run_ref_calibration(void *device_data)
{
	...
	ret = ztm730_hw_calibration(info);
...
#if SUPPORTED_TOUCH_WHEEL
static ssize_t ztm730_store_wheel_enable(struct device *dev,
				  ...
				  const char *buf, size_t size)
{
	...
	ret = ztm730_set_optional_mode(info, true);
	if (ret) {
		dev_err(&client->dev, "%s:failed ztm730_set_optional_mode\n", __func__);
		return -EIO;
	}
...
static ssize_t ztm730_show_wheel_enable(struct device *dev,
...
static ssize_t ztm730_store_glove_enable(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t size)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
...
static ssize_t ztm730_show_touch_active(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
...
static ssize_t ztm730_show_glove_enable(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
...
static ssize_t ztm730_show_gesture_mode(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
...
static ssize_t ztm730_store_gesture_mode(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t size)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
...
gesture_retry:
	...
	if (enable) {
		ret = ztm730_store_gesture_on(info);
		if (ret) {
		...
		ret = ztm730_store_gesture_off(info);
		if (ret) {
...
static ssize_t ztm730_show_hard_reset(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
...
static ssize_t ztm730_show_debug_enable(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
...
static ssize_t ztm730_store_debug_enable(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t size)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
...
static ssize_t ztm730_store_read_reg(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t size)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
...
static ssize_t ztm730_show_read_reg(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	...
	ret = ztm730_read_data(info->client, (u16)read_reg_addr, (u8 *)&r_buff[0], read_reg_size);
	if (ret < 0) {
		dev_err(&client->dev, "%s:failed read params[%d]\n", __func__, ret);
		return -EIO;
	}
...
static ssize_t ztm730_store_write_reg(struct device *dev,
					  struct device_attribute *attr,
					  const char *buf, size_t size)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	...
	ret = ztm730_write_reg(info->client, (u16)buff[0], (u16)buff[2]);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s:Failed to write ZTM730_REGISTER_WRITE(%x)(%x)\n",
			__func__, buff[0], buff[1]);
		return -EIO;
	}
...
static ssize_t ztm730_show_fw_ver_ic(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	char buffer[32] = {0, };
...
static ssize_t ztm730_show_fw_ver_bin(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	char buffer[32] = {0, };
...
static ssize_t ztm730_show_multi_touch_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_wet_mode_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_noise_mode_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_chip_code(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_comm_err_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_hard_covery_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_vendor(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;

	dev_info(&client->dev, "%s:[%s]\n", __func__, ZTM730_VENDOR_NAME);
...
static ssize_t ztm730_show_fw_checksum(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_all_touch_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_aod_touch_cnt(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_holding_time(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_gesture_time(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_normal_time(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static ssize_t ztm730_show_raw_check(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int i, ret;
...
static ssize_t ztm730_show_max_z_value(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct ztm730_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static DEVICE_ATTR(z_value, S_IRUGO, ztm730_show_max_z_value, NULL);
static DEVICE_ATTR(raw_check, S_IRUGO, ztm730_show_raw_check, NULL);
static DEVICE_ATTR(gesture_time, S_IRUGO, ztm730_show_gesture_time, NULL);
static DEVICE_ATTR(normal_time, S_IRUGO, ztm730_show_normal_time, NULL);
static DEVICE_ATTR(holding_time, S_IRUGO, ztm730_show_holding_time, NULL);
static DEVICE_ATTR(aod_touch_count, S_IRUGO, ztm730_show_aod_touch_cnt, NULL);
static DEVICE_ATTR(all_touch_count, S_IRUGO, ztm730_show_all_touch_cnt, NULL);
static DEVICE_ATTR(recovery_count, S_IRUGO, ztm730_show_hard_covery_cnt, NULL);
static DEVICE_ATTR(checksum, S_IRUGO, ztm730_show_fw_checksum, NULL);
static DEVICE_ATTR(vendor, S_IRUGO, ztm730_show_vendor, NULL);
static DEVICE_ATTR(comm_err_count, S_IRUGO, ztm730_show_comm_err_cnt, NULL);
static DEVICE_ATTR(module_id, S_IRUGO, ztm730_show_chip_code, NULL);
static DEVICE_ATTR(noise_mode, S_IRUGO, ztm730_show_noise_mode_cnt, NULL);
static DEVICE_ATTR(wet_mode, S_IRUGO, ztm730_show_wet_mode_cnt, NULL);
static DEVICE_ATTR(multi_check, S_IRUGO, ztm730_show_multi_touch_cnt, NULL);
static DEVICE_ATTR(fw_ver_ic, S_IRUGO, ztm730_show_fw_ver_ic, NULL);
static DEVICE_ATTR(fw_ver_bin, S_IRUGO, ztm730_show_fw_ver_bin, NULL);
static DEVICE_ATTR(mode, S_IRUGO | S_IWUSR | S_IWGRP/*nyS_IRUGO | S_IWGRP*/,\
		ztm730_show_gesture_mode, ztm730_store_gesture_mode);
static DEVICE_ATTR(cmd, S_IWUSR | S_IWGRP/*nyS_IWGRP*/, NULL, store_cmd);
static DEVICE_ATTR(cmd_status, S_IRUGO, show_cmd_status, NULL);
static DEVICE_ATTR(cmd_result, S_IRUGO, show_cmd_result, NULL);
static DEVICE_ATTR(glove_mode, S_IRUGO | S_IWUSR | S_IWGRP,
			ztm730_show_glove_enable, ztm730_store_glove_enable);
static DEVICE_ATTR(touch_active, S_IRUGO, ztm730_show_touch_active, NULL);
static DEVICE_ATTR(debug, S_IRUGO | S_IWUSR | S_IWGRP,\
		ztm730_show_debug_enable, ztm730_store_debug_enable);
static DEVICE_ATTR(reset, S_IRUGO, ztm730_show_hard_reset, NULL);
static DEVICE_ATTR(read_reg, S_IRUGO | S_IWUSR | S_IWGRP,\
		ztm730_show_read_reg, ztm730_store_read_reg);
static DEVICE_ATTR(write_reg, S_IWUSR | S_IWGRP,\
		NULL, ztm730_store_write_reg);
#if SUPPORTED_TOUCH_WHEEL
static DEVICE_ATTR(wheel, S_IRUGO | S_IWUSR | S_IWGRP,
			ztm730_show_wheel_enable, ztm730_store_wheel_enable);
#endif
...
static int ztm730_init_sec_factory(struct ztm730_info *info)
{
...
static int ztm730_ts_parse_dt(struct device_node *np, struct device *dev,
	...
	unsigned int cmd, unsigned long arg)
{
...
#endif
	if (misc_info == NULL) {
		dev_err(&misc_info->client->dev, "%s:misc device NULL?\n", __func__);
		return -1;
	}
	...
	case TOUCH_IOCTL_SET_DEBUGMSG_STATE:
		if (copy_from_user(&nval, argp, sizeof(nval))) {
			dev_err(&misc_info->client->dev, "%s:copy_from_user\n", __func__);
			return -1;
		}
		if (nval)
			dev_err(&misc_info->client->dev, "%s:on debug mode (%d)\n", __func__, nval);
		else
			dev_err(&misc_info->client->dev, "%s:off debug mode (%d)\n", __func__, nval);
	...
	case TOUCH_IOCTL_VARIFY_UPGRADE_SIZE:
		...
		dev_info(&misc_info->client->dev, "%s:firmware size = %d\n", __func__, (int)sz);
		if (misc_info->cap_info.ic_fw_size != sz) {
			dev_err(&misc_info->client->dev, "%s:firmware size error\n", __func__);
			return -1;
		}
		break;
/*
...
*/
	case TOUCH_IOCTL_START_UPGRADE:
	{
		...
		if (copy_from_user(&raw_ioctl,
			...
			up(&misc_info->raw_data_lock);
			dev_err(&misc_info->client->dev, "%s:copy_from_user(2)\n", __func__);
			return -1;
		}
		...
		if (copy_from_user((u8 *)buff,(void __user *)(raw_ioctl.buf), size)) {
			...
			dev_err(&misc_info->client->dev, " %s:copy_from_user(3)\n", __func__);
			return -1;
		}
	...
	case TOUCH_IOCTL_HW_CALIBRAION:
		...
		if ( ret != I2C_SUCCESS) {
			dev_err(&misc_info->client->dev, "%s:failed to set touch mode %d.\n", __func__, mode);
			goto fail_hw_cal;
		}

		ret = ztm730_soft_reset(misc_info);
		if (ret) {
			dev_err(&misc_info->client->dev, "%s:Failed to write reset command\n", __func__);
			goto fail_hw_cal;
		}
...
fail_hw_cal:
	...
	case TOUCH_IOCTL_SET_RAW_DATA_MODE:
		...
		if (ret) {
			dev_err(&misc_info->client->dev, "%s:Failed to set POINT_MODE\n", __func__);
			misc_info->work_state = NOTHING;
			return -1;
		}
	...
	case TOUCH_IOCTL_GET_REG:
		...
		if (copy_from_user(&reg_ioctl,
			...
			misc_info->work_state = NOTHING;
			up(&misc_info->work_lock);
			dev_err(&misc_info->client->dev, " %s:copy_from_user(1)\n", __func__);
			return -1;
		}
		...
		if (copy_to_user((void __user *)(reg_ioctl.val), (u8 *)&nval, 4)) {
			...
			misc_info->work_state = NOTHING;
			up(&misc_info->work_lock);
			dev_err(&misc_info->client->dev, " %s:copy_to_user(2)\n", __func__);
			return -1;
		}

		dev_err(&misc_info->client->dev, "%s:reg addr = 0x%x, val = 0x%x\n",
	...
	case TOUCH_IOCTL_SET_REG:
		...
		dev_err(&misc_info->client->dev, "%s:write: reg addr = 0x%x, val = 0x%x\n",
	...
	case TOUCH_IOCTL_DONOT_TOUCH_EVENT:
		...
		if ( ret != I2C_SUCCESS) {
			dev_err(&misc_info->client->dev, "%s:failed to set ZTM730_INT_ENABLE_FLAG.\n", __func__);
			ret = -1;
		}
		dev_err(&misc_info->client->dev, "%s:write: reg addr = 0x%x, val = 0x0\n",
	...
	case TOUCH_IOCTL_SEND_SAVE_STATUS:
		...
		if (ret != I2C_SUCCESS) {
			dev_err(&misc_info->client->dev, "%s:failed to write ZTM730_SAVE_STATUS_CMD\n", __func__);
			ret =  -1;
		}
...
static void ztm730_setup_input_device(struct ztm730_info *info)
{
...
static int ztm730_ts_gpio_init(struct ztm730_info *info)
{
	struct i2c_client *client = info->client;
	struct zxt_ts_platform_data	*pdata = info->pdata;
	int ret;
	...
	if (ret) {
		dev_err(&client->dev, "%s:Failed ztm730_pinctrl_configure\n", __func__);
		ret = -EINVAL;
		goto out;
	}
#endif
...
/*
static void ztm730_debug_dwork(struct work_struct *work)
{
...
*/
#if SUPPORTED_TOUCH_WHEEL
static int ztm730_bezel_open(struct input_dev *dev)
{
	struct ztm730_info *info = input_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
...
static void ztm730_bezel_close(struct input_dev *dev)
{
	struct ztm730_info *info = input_get_drvdata(dev);
	struct i2c_client *client = info->client;
	int ret;
			...
			struct device_attribute *attr, char *buf)
{
	...
	ret = ztm730_read_data(info->client, ZTM730_WHEEL_VIB_SETTING, (u8 *)&status, 2);
	if (ret < 0) {
...
static int ztm730_bezel_sysfs_init(struct ztm730_info *info)
{
	struct ztm_bezel_ddata *ddata = info->bezel_info;
	struct i2c_client *client = info->client;
	int ret = 0;
...
static int ztm730_setup_bezel_input(struct ztm730_info *info)
{
	struct ztm_bezel_ddata *ddata = info->bezel_info;
	struct i2c_client *client = info->client;
	int ret;
	...
	/* need to check fail */
	ret = ztm730_bezel_sysfs_init(info);
	if (ret) {
		dev_err(&client->dev, "%s:failed ztm730_bezel_sysfs_init[%d]\n",\
			__func__, ret);
//		return -EPERM;	//temp
	}
...
static int ztm730_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
	...
	if (client->dev.of_node) {
		...
		if (ret) {
			dev_err(&client->dev, "%s:Failed ztm730_ts_parse_dt\n", __func__);
			goto err_no_platform_data;
		}
	}
	...
	if (ret) {
		dev_err(&client->dev, "%s:Failed to ztm730_ts_gpio_init\n", __func__);
		goto err_alloc;
	}
...
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_register_ops(info, &ztm730_sleep_monitor_ops,
			SLEEP_MONITOR_TSP);
#endif
...
#ifdef SEC_FACTORY_TEST
	ret = ztm730_init_sec_factory(info);
	if (ret) {
...
static int ztm730_remove(struct i2c_client *client)
{
	struct ztm730_info *info = i2c_get_clientdata(client);
	struct zxt_ts_platform_data *pdata = info->pdata;
...
static struct i2c_device_id ztm730_idtable[] = {
	{ZTM730_TS_DEVICE, 0},
	{ }
};
...
static struct i2c_driver ztm730_ts_driver = {
	.probe	= ztm730_probe,
...
static int __init ztm730_init(void)
{
	int ret;
...
static void __exit ztm730_exit(void)
{
