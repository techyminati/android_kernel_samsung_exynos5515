					    ...
					    struct spi_message *msg)
{
	...
	unsigned target_len = 0, origin_len = 0, origin_bpw = 0;
...
try_transfer:
		...
		/* Polling method for xfers not bigger than FIFO capacity */
			if (xfer->len <= fifo_lvl) {
				if (sdd->cur_bpw != origin_bpw) {
					sdd->cur_bpw = origin_bpw;
					s3c64xx_spi_config(sdd);
				}
				use_dma = 0;
			} else {
				/* Align Burst size 16byte */
				xfer->len = xfer->len & ~0xf;
				if (sdd->cur_bpw != 32) {
					sdd->cur_bpw = 32;
					s3c64xx_spi_config(sdd);
				}
