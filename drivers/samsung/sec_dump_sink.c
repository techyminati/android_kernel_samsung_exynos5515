...
#define ENABLE_SELECT_RAMDUMP		(0x65254478)
#define MAGIC_SEL_FOR_MINFORM		(0xD)
...
static int sec_sdcard_ramdump(const char *val, const struct kernel_param *kp)
{
	...
	} else if (dump_sink == ENABLE_SELECT_RAMDUMP) {
		sec_set_reboot_magic(MAGIC_SEL_FOR_MINFORM, OFFSET_SDR_FOR_MINFORM, MASK_SDR_FOR_MINFORM);
	}
	return 0;
}
