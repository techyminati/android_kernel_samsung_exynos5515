...
static const char * const dss_freq_name[] = {
	"LIT", "MIF", "INT", "DISP", "AUD", "G3D"
};
...
static void secdbg_hardlockup_show_freq(struct hardlockup_info *hl_info)
{
	...
	for (i = 0; i < ARRAY_SIZE(dss_freq_name); i++) {
...
static void secdbg_hardlockup_show_info(struct hardlockup_info *hl_info)
{
	...
	case HL_TASK_STUCK:
		offset += scnprintf(buf + offset, PRINT_LINE_MAX - offset, " task=%s]", hl_info->task_info.task_comm);
		break;
	case HL_IRQ_STUCK:
...
late_initcall_sync(secdbg_hardlockup_info_init);
