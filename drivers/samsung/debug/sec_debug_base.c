...
static struct attribute *recovery_cause_attributes[] = {
	&dev_attr_recovery_cause.attr,
	NULL,
};

static struct attribute_group recovery_cause_attr_group = {
	.attrs = recovery_cause_attributes,
	NULL,
};
...
static int __init sec_debug_recovery_cause_init(void)
{
	...
	ret = sysfs_create_group(&secdbg_dev->kobj, &recovery_cause_attr_group);
	if (ret)
