/*
 * linux/drivers/video/fbdev/exynos/panel/s6e36w4x01/s6e36w4x01.c
 *
...
static void show_rddpm(struct dumpinfo *info)
{
	...
	u8 rddpm[S6E36W4X01_RDDPM_LEN] = { 0, };
...
static void show_rddsm(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 rddsm[S6E36W4X01_RDDSM_LEN] = { 0, };
#ifdef CONFIG_LOGGING_BIGDATA_BUG
...
static void show_err(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 err[S6E36W4X01_ERR_LEN] = { 0, }, err_15_8, err_7_0;
...
{
	int ret;
	u8 err_fg[S6E36W4X01_ERR_FG_LEN] = { 0, };
	struct resinfo *res = info->res;
...
static void show_dsi_err(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 dsi_err[S6E36W4X01_DSI_ERR_LEN] = { 0, };
...
static void show_self_diag(struct dumpinfo *info)
{
	int ret;
	struct resinfo *res = info->res;
	u8 self_diag[S6E36W4X01_SELF_DIAG_LEN] = { 0, };
