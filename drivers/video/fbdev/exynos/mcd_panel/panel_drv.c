...
#endif
	[PANEL_WORK_FIRST_FRAME] = "first_frame",
};
...
static char *panel_gpio_names[PANEL_GPIO_MAX] = {
	...
	[PANEL_GPIO_AVDD_EN] = PANEL_GPIO_NAME_AVDD_EN,
};
...
static int __init get_boot_hw_revision(char *arg)
{
	get_option(&arg, &boot_hw_revision);
	panel_info("boot_hw_revision : %d\n", boot_hw_revision);

	return 0;
}
early_param("androidboot.revision", get_boot_hw_revision);
...
static int panel_regulator_enable(struct panel_device *panel)
{
	...
	if (gpio_is_valid(gpio[PANEL_GPIO_AVDD_EN].num))
		gpio_direction_output(gpio[PANEL_GPIO_AVDD_EN].num, 1);
...
static int panel_regulator_disable(struct panel_device *panel)
{
	...
	if (gpio_is_valid(gpio[PANEL_GPIO_AVDD_EN].num))
		gpio_direction_output(gpio[PANEL_GPIO_AVDD_EN].num, 0);
...
static int __panel_seq_init(struct panel_device *panel)
{
	...
	if (check_seqtbl_exist(&panel->panel_data, PANEL_SLEEP_OUT_SEQ)) {
		ret = panel_do_seqtbl_by_index_nolock(panel, PANEL_SLEEP_OUT_SEQ);
		if (unlikely(ret < 0)) {
			panel_err("failed to write sleep-out seqtbl\n");
			goto err_init_seq;
		}
	}
...
#endif
	panel_data->test_key = info->test_key;
	mutex_unlock(&panel->op_lock);
...
int panel_probe(struct panel_device *panel)
{
	...
	mutex_lock(&panel->op_lock);
	panel_data->board_revision = boot_hw_revision;
...
//minwoo7945.kim todo wise does not support disp_det
		det_val = 1;
	} else {
...
static int panel_parse_gpio(struct panel_device *panel)
{
	...
	for_each_child_of_node(gpios_np, np) {
		for (i = 0; i < PANEL_GPIO_MAX; i++)
			if (!of_node_cmp(np->name, panel_gpio_names[i]))
				break;
...
int panel_register_isr(struct panel_device *panel)
{
	...
	for (i = 0; i < PANEL_GPIO_MAX; i++) {
		...
		for (iw = 0; iw < PANEL_WORK_MAX; iw++) {
			if( (panel_gpio_names[i] == NULL) || (panel_work_names[iw] == NULL))
				continue;
...
static void disp_det_handler(struct work_struct *work)
{
	...
	case PANEL_STATE_NORMAL:
		if (disp_det_state == PANEL_STATE_NOK) {
			...
			ret = panel_isr_do_action(panel, PANEL_ISR_DISP_DET, 0);
...
void conn_det_handler(struct work_struct *data)
{
	...
	ret = panel_isr_do_action(panel, PANEL_ISR_CONN_DET, (u32)is_disconnected);
	if (ret)
		panel_err("failed to conn_det exec\n");
...
void err_fg_handler(struct work_struct *data)
{
	...
	struct panel_gpio *gpio = panel->gpio;
	struct panel_state *state = &panel->state;
	bool is_disconnected = ub_con_disconnected(panel);

	err_fg_state = panel_err_fg_state(panel);
	panel_info("err_fg_state:%s, connect_state:%s\n",
		err_fg_state == PANEL_STATE_OK ? "OK" : "NOK",
		is_disconnected == true ? "DISCONNECT" : "CONNECT");
	...
	case PANEL_STATE_NORMAL:
		if (err_fg_state == PANEL_STATE_NOK) {
			if (!is_disconnected) {
				...
				ret = panel_isr_do_action(panel, PANEL_ISR_ERR_FG, 1);
...
int panel_isr_do_action(struct panel_device *panel, u32 handler_type, u32 state)
{
	u32 isr_action = 0;
	int ret = 0;

	if (handler_type >= MAX_PANEL_ISR) {
		//error
		panel_err("handler type mismatched %d\n", handler_type);
		return -EINVAL;
	}

	isr_action = panel->panel_data.ddi_props.isr_action[handler_type];
	panel_info("handler %d state %d isr_action 0x%02X\n", handler_type, state, isr_action);

	if (isr_action & PANEL_ISR_ACTION_RECOVERY) {
		panel->state.dump_bypass = true;
		ret = panel_error_cb(panel);
		if (ret)
			panel_err("failed to error_cb\n");
		panel->state.dump_bypass = false;
	}
	if (isr_action & PANEL_ISR_ACTION_POWERDOWN) {
		panel->state.dump_bypass = true;
	...
	if (isr_action & PANEL_ISR_ACTION_UBCONN_NOTIFY) {
#ifdef CONFIG_PANEL_NOTIFY
		panel_send_ubconn_notify((state ?
			PANEL_EVENT_UB_CON_DISCONNECTED : PANEL_EVENT_UB_CON_CONNECTED));
#endif
	}
	if (isr_action & PANEL_ISR_ACTION_UBCONN_UEVENT) {
		if (state)
			panel_send_ubconn_uevent(panel);
	}
	if (isr_action & PANEL_ISR_ACTION_UBCONN_ABC) {
		if (state) {
#if IS_ENABLED(CONFIG_SEC_ABC)
...
int panel_device_init(struct panel_device *panel)
{
	...
	panel->state.power = PANEL_POWER_OFF;
	panel->state.disp_on = PANEL_DISPLAY_OFF;
	panel->ktime_panel_on = panel->ktime_panel_disp_on = ktime_get();
