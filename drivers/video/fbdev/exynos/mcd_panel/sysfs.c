...
static ssize_t cell_id_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	u8 date[PANEL_DATE_LEN] = { 0, }, coordinate[4] = { 0, }, product_info[11] = { 0, };
	...
	if ((panel_data->vendor[0] == 'B') && (panel_data->vendor[1] == 'O') &&
		(panel_data->vendor[2] == 'E')) {
		resource_copy_by_name(panel_data, &product_info[0], "product_info1");
		resource_copy_by_name(panel_data, &product_info[1], "product_info2");
		resource_copy_by_name(panel_data, &product_info[2], "product_info3");
		resource_copy_by_name(panel_data, &product_info[3], "product_info4");
		resource_copy_by_name(panel_data, &product_info[4], "product_info5");
		resource_copy_by_name(panel_data, &product_info[5], "product_info6");
		resource_copy_by_name(panel_data, &product_info[6], "product_info7");
		resource_copy_by_name(panel_data, &product_info[7], "product_info8");
		resource_copy_by_name(panel_data, &product_info[8], "product_info9");
		resource_copy_by_name(panel_data, &product_info[9], "product_info10");
		resource_copy_by_name(panel_data, &product_info[10], "product_info11");

		snprintf(buf, PAGE_SIZE, "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X\n",
			product_info[0], product_info[1], product_info[2], product_info[3],
			product_info[4], product_info[5], product_info[6], product_info[7],
			product_info[8], product_info[9], product_info[10]);
	} else {
		pr_info("%s sdc\n", __func__);
	...
	struct device_attribute *attr, char *buf)
{
	...
	if ((panel_data->vendor[0] == 'B') && (panel_data->vendor[1] == 'O') &&
		(panel_data->vendor[2] == 'E')) {
		resource_copy_by_name(panel_data, &date[0], "product_info1");
		resource_copy_by_name(panel_data, &date[1], "product_info2");
		resource_copy_by_name(panel_data, &date[2], "product_info3");
		resource_copy_by_name(panel_data, &date[3], "product_info4");
		resource_copy_by_name(panel_data, &date[4], "product_info5");
		resource_copy_by_name(panel_data, &date[5], "product_info6");

		snprintf(buf, PAGE_SIZE, "%d%d, %d, %d, %d:%d\n",
...
static ssize_t errfg_count_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct panel_info *panel_data;
	struct panel_device *panel = dev_get_drvdata(dev);
...
static ssize_t errfg_count_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	int rc;
	u32 value;
...
#endif
	__PANEL_ATTR_RW(errfg_count, 0664),
};
