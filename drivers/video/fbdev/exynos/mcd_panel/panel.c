...
static struct common_panel_info *find_common_panel_with_name(const char *name)
{
	...
	panel_info("find panel with name: %s\n", name);
	...
	for (i = 0; i < nr_panel; i++) {
		...
		panel_info("list name: %s, name: %s", panel_list[i]->name, name);
...
void print_panel_lut(struct panel_lut_info *lut_info)
{
	...
	for (i = 0; i < lut_info->nr_panel; i++)
		panel_info("panel_lut names[%d] %s\n", i, lut_info->names[i]);

	for (i = 0; i < lut_info->nr_lut; i++)
		panel_info("panel_lut[%d] id:0x%08X mask:0x%08X index:%d(%s)\n",
...
int find_panel_ddi_lut(struct panel_device *panel, u32 id)
{
	...
	panel_info("%s: nr_lut: %d\n", __func__, lut_info->nr_lut);
...
#if defined(CONFIG_DISPLAY_VRR)
	current_fps = panel->panel_data.props.vrr_fps;
#else
	//minwoo7945.kim fps
	current_fps = 60;
#endif
...
#if defined (CONFIG_MIPI_SRRAM_WRITE)
...
{
	int ret = 0;
	struct panel_test_key_info *key;
	...
	key = &panel->panel_data.test_key;
	if (key->len == 0) {
		panel_err("key len is 0\n");
		return 0;
	}

	if (on) {
		if ((level >= 1) && (key->level1_on != NULL)) {
			ret = panel_dsi_write_cmd(panel, MIPI_DSI_WR_GEN_CMD,
					key->level1_on, 0, key->len, false);
			if (ret != key->len) {
				panel_err("fail to write CMD : TEST_KEY_LEVEL1_ON\n");
				return -EIO;
			}
		}

		if ((level >= 2) && (key->level2_on != NULL)) {
			ret = panel_dsi_write_cmd(panel, MIPI_DSI_WR_GEN_CMD,
					key->level2_on, 0, key->len, false);
			if (ret != key->len) {
				panel_err("fail to write CMD : TEST_KEY_LEVEL2_ON\n");
				return -EIO;
			}
		}

		if ((level >= 3) && (key->level3_on != NULL)) {
			ret = panel_dsi_write_cmd(panel, MIPI_DSI_WR_GEN_CMD,
					key->level3_on, 0, key->len, false);
			if (ret != key->len) {
				panel_err("fail to write CMD : TEST_KEY_LEVEL3_ON\n");
				return -EIO;
			}
		}
	} else {
		if ((level >= 3) && (key->level3_off != NULL)) {
			ret = panel_dsi_write_cmd(panel, MIPI_DSI_WR_GEN_CMD,
					key->level3_off, 0,  key->len, false);
			if (ret != key->len) {
				panel_err("fail to write CMD : TEST_KEY_LEVEL3_OFF\n");
				return -EIO;
			}
		}

		if ((level >= 2) && (key->level2_off != NULL)) {
			ret = panel_dsi_write_cmd(panel, MIPI_DSI_WR_GEN_CMD,
					key->level2_off, 0,  key->len, false);
			if (ret != key->len) {
				panel_err("fail to write CMD : TEST_KEY_LEVEL2_OFF\n");
				return -EIO;
			}
		}

		if ((level >= 1) && (key->level1_off != NULL)) {
			ret = panel_dsi_write_cmd(panel, MIPI_DSI_WR_GEN_CMD,
					key->level1_off, 0,  key->len, false);
			if (ret != key->len) {
				panel_err("fail to write CMD : TEST_KEY_LEVEL1_OFF\n");
				return -EIO;
			}
		}
	}
...
static int panel_do_tx_packet(struct panel_device *panel, struct pktinfo *info, bool block)
{
	...
	case DSI_PKT_TYPE_WR_MEM:
		...
		break;
#else
		panel_err("%s: not supoort DSI_PKT_TYPE_WR_SR type\n", __func__);
		return -EINVAL;
#endif	
...
int read_panel_id(struct panel_device *panel, u8 *buf)
{
	...
	mutex_lock(&panel->op_lock);
	/* remove test key.. panel id does not require test key */
...
int panel_dumpinfo_update(struct panel_device *panel, struct dumpinfo *info)
{
	...
	if (panel->state.dump_bypass) {
		panel_info("bypass dump command\n");
		return 0;
	}
