...
static int __init exynos_pd_late_init(void)
{
	struct device_node *np = NULL;
	for_each_compatible_node(np, NULL, "samsung,exynos-pd") {
		struct platform_device *pdev;
		...
		ret = of_property_read_string(np, "regulator_name", &name);
		if (!ret) {
			pd->rgt = regulator_get(&pdev->dev, name);
			if (pd->rgt)
...
late_initcall(exynos_pd_late_init);
