...
EXPORT_SYMBOL(dbg_snapshot_expire_watchdog_timeout);

int dbg_snapshot_expire_watchdog(void)
{
	dbg_snapshot_expire_watchdog_timeout(INT_MAX);
...
extern unsigned long sea_incorrect_addr;

static bool secdbg_is_in_sea_area(void *target_addr, int area_offset)
{
	unsigned long taddr = (unsigned long)target_addr;

	return (sea_incorrect_addr &&
		(taddr >= (sea_incorrect_addr - area_offset)) &&
		(taddr <= (sea_incorrect_addr + area_offset)));
}
#endif
...
static void show_data(unsigned long addr, int nbytes, const char *name)
{
	...
	for (i = 0; i < nlines; i++) {
		...
		for (j = 0; j < 8; j++, p++) {
#if IS_ENABLED(CONFIG_SEC_DEBUG_AVOID_UNNECESSARY_TRAP)
			if (secdbg_is_in_sea_area(p, nbytes)) {
				pr_cont(" --------");
				continue;
			}
#endif
...
void dbg_snapshot_init_utils(void)
{
	...
	if (secdbg_mode_enter_upload()) {
		...
		dev_info(dss_desc.dev, "%s: force upload key registered\n", __func__);
	}
