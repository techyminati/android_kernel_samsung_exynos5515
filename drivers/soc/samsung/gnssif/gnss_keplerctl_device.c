...
static irqreturn_t kepler_wdt_isr(int irq, void *arg)
{
	...
	gif_info("disable gnss wdt interrupt\n");
    gif_disable_irq_nosync(&gc->irq_gnss_wdt);
#endif

/*	
	#if IS_ENABLED(CONFIG_DEBUG_SNAPSHOT)
	gif_err("reset_count:%d\n", gc->reset_count);
	if (gc->reset_count >= 100) {
		gif_err("dbg_snapshot_expire_watchdog\n");
...
static int kepler_hold_reset(struct gnss_ctl *gc)
{
	...
	if (gpio_is_valid(gc->pdata->gpio_rfic_reset)) {
		gpio_set_value(gc->pdata->gpio_rfic_reset, 0);
...
static int kepler_release_reset(struct gnss_ctl *gc)
{
	...
	if (gpio_is_valid(gc->pdata->gpio_rfic_reset)) {
		gpio_set_value(gc->pdata->gpio_rfic_reset, 1);
		udelay(GNSS_DELAY_SET_RFIC);
		gpio_set_value(gc->pdata->gpio_rfic_reset, 0);
		udelay(GNSS_DELAY_SET_RFIC);
		gpio_set_value(gc->pdata->gpio_rfic_reset, 1);
	...
	gc->pmu_ops->req_baaw();
#if !defined(CONFIG_SENSORS_SSP_WISE) && !defined(CONFIG_SENSORS_SSP_FRESH)
	gif_info("enable gnss wdt interrupt\n");
...
static int kepler_power_on(struct gnss_ctl *gc)
{
	...
	if (gpio_is_valid(gc->pdata->gpio_rfic_reset)) {
		gpio_direction_output(gc->pdata->gpio_rfic_reset, 0);
		udelay(GNSS_DELAY_SET_RFIC);
		gpio_set_value(gc->pdata->gpio_rfic_reset, 1);
		udelay(GNSS_DELAY_SET_RFIC);
		gpio_set_value(gc->pdata->gpio_rfic_reset, 0);
		...
		unsigned long action, void *nb_data)
{
	...
	if (IS_ERR_OR_NULL(itmon_data))
		return NOTIFY_DONE;
	if (itmon_data->port &&
		(strncmp("BUS1", itmon_data->port, sizeof("BUS1") - 1) == 0) &&
		itmon_data->master &&
		...
		return NOTIFY_OK;
	} else if (itmon_data->dest &&
		(strncmp("GNSS", itmon_data->dest, sizeof("GNSS") - 1) == 0)) {
		return NOTIFY_BAD;
	}
	return NOTIFY_DONE;
}
