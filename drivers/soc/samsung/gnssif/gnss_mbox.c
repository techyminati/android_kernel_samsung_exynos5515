...
static int gnss_mbox_suspend(struct device *dev)
{
	return 0;
}

static int gnss_mbox_resume(struct device *dev)
{
	unsigned int wakeup_stat;

	exynos_pmu_read(0x3c50, &wakeup_stat);	/* WAKEUP_STAT */
	if (wakeup_stat & (1 << 13))	/* MAILBOX_GNSS2AP */
		log_irq_wakeup_reason(_gnss_mbox[0].irq_gnss_mbox.num);
...
static const struct dev_pm_ops gnss_mbox_pm_ops = {
	.suspend = gnss_mbox_suspend,
	.resume = gnss_mbox_resume,
};
