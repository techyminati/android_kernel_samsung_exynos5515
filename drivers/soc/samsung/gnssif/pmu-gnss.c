...
#define BAAW_GNSS_CMGP_ADDR	(0x115E0000)
#define BAAW_GNSS_CMGP_SIZE	(SZ_64K)

#define BAAW_GNSS_DBUS_ADDR	(0x115D0000)
...
#if defined(CONFIG_SOC_EXYNOS9630) || defined(CONFIG_SOC_EXYNOS3830) || defined(CONFIG_SOC_S5E5515)
...
#if defined(CONFIG_SOC_S5E5515)
	gif_info("MAILBOX CP APM AP CHUB WLBT configuration\n");
	gnss_cmgp_write(0x00, 0x000B0F60);	/* GNSS Start address >> 12bit */
	gnss_cmgp_write(0x04, 0x000B0FB0);	/* GNSS End address >> 12bit */
	gnss_cmgp_write(0x08, 0x00012970);	/* AP Start address >> 12bit */
	gnss_cmgp_write(0x0C, 0x80000003);

	gif_info("CHUB_SRAM non cachable\n");
	gnss_cmgp_write(0x10, 0x000B1A00);
	gnss_cmgp_write(0x14, 0x000B1A01);
	gnss_cmgp_write(0x18, 0x00010DFF);
	gnss_cmgp_write(0x1C, 0x80000003);

	gif_info("GPIO_CMGP\n");
	gnss_cmgp_write(0x20, 0x000B2B30);
	gnss_cmgp_write(0x24, 0x000B2B40);
	gnss_cmgp_write(0x28, 0x00012B30);
