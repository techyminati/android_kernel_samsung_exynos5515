...
static struct cpumask create_cpumask(void)
{
	...
	online_cpu_min = max(online_cpu_min, (int)cpumask_weight(cpu_online_mask));
...
static void __init cpu_hotplug_pm_qos_init(void)
{
	...
	cpu_hotplug.user_min = PM_QOS_CPU_ONLINE_MAX_DEFAULT_VALUE;
	cpu_hotplug.user_max = PM_QOS_CPU_ONLINE_MAX_DEFAULT_VALUE;
	pm_qos_add_request(&user_min_cpu_hotplug_request, PM_QOS_CPU_ONLINE_MIN, cpu_hotplug.user_min);
	pm_qos_add_request(&user_max_cpu_hotplug_request, PM_QOS_CPU_ONLINE_MAX, cpu_hotplug.user_max);
}
