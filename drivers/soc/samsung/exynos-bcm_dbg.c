...
/* disable PPMU recording for DPU block when LPD scenario */
static unsigned int dpu_cal_pdid;

static int lpd_noti_start_fn(struct notifier_block *nb, unsigned long action, void *data)
{
	int ret = 0;
	if(bcm_dbg_data == NULL)
		return NOTIFY_OK;
	if(bcm_dbg_data->bcm_load_bin == false)
		return NOTIFY_OK;

	if(action == LPD_NOTIFIER_START) {
		ret = exynos_bcm_dbg_pd_sync(dpu_cal_pdid, false);
	}

	return NOTIFY_OK;
}

static struct notifier_block lpd_start_nb = {
	.notifier_call = lpd_noti_start_fn,
	.priority = 7,
};

static int lpd_noti_stop_fn(struct notifier_block *nb, unsigned long action, void *data)
{
	int ret = 0;
	if(bcm_dbg_data == NULL)
		return NOTIFY_OK;
	...
	if (action == LPD_NOTIFIER_STOP) {
		ret = exynos_bcm_dbg_pd_sync(dpu_cal_pdid, true);
	}

	return NOTIFY_OK;
}

static struct notifier_block lpd_stop_nb = {
	.notifier_call = lpd_noti_stop_fn,
	.priority = 3,
};
#endif
...
{
	int ret = 0;
#if 0 //defined(CONFIG_EXYNOS_ADV_TRACER) || defined(CONFIG_EXYNOS_ADV_TRACER_MODULE)
	int i = 0;
	struct adv_tracer_ipc_cmd config;
#endif
#if 1 //defined(CONFIG_EXYNOS_BCM_DBG_GNR) || defined(CONFIG_EXYNOS_BCM_DBG_GNR_MODULE)
	int i = 0;
...
static int exynos_bcm_dbg_pd_sync_init(struct exynos_bcm_dbg_data *data)
{
	...
	for (pd_index = 0; pd_index < pd_size; pd_index++) {
		...
		if (exynos_pd) {
			...
			#if defined(CONFIG_SOC_S5E5515) && defined(CONFIG_EXYNOS_LPD)
			if(strcmp(data->pd_info[pd_index]->pd_name, "pd-disp") == 0)
				dpu_cal_pdid = exynos_pd->cal_pdid;
			#endif
...
int __nocfi exynos_bcm_dbg_load_bin(void)
{
	...
	ret = bcm_change_memory_common((unsigned long)bcm_early_vm.addr,
				BCM_SIZE, __pgprot(0), __pgprot(PTE_PXN));
	if (ret) {
...
#if defined(CONFIG_SOC_S5E5515) && defined(CONFIG_EXYNOS_LPD)
	lpd_notifier_register(&lpd_start_nb);
	lpd_notifier_register(&lpd_stop_nb);
#endif
