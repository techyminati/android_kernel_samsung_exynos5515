...
#if defined(CONFIG_SEC_PM) && defined(CONFIG_MUIC_NOTIFIER)
static struct kobj_attribute sicd_block_jig_attr;
static bool sicd_block_jig; /* blocking SICD by jig */
static bool jig_is_attached;
static bool is_jig_attached(void)
{
	if (sicd_block_jig && jig_is_attached)
		return true;
...
static int can_enter_power_mode(int cpu, struct power_mode *mode)
{
	...
	if (mode->type == POWERMODE_TYPE_SYSTEM && is_jig_attached())
		return 0;
...
#if defined(CONFIG_SEC_PM) && defined(CONFIG_MUIC_NOTIFIER)
	if (attr_count < 10) { /* static struct attribute *attr_pool[10] */
		cpupm_attr_init(sicd_block_jig_attr, "sicd_block_jig", 0644,
			show_sicd_block_jig, store_sicd_block_jig);
		attr_pool[attr_count++] = &sicd_block_jig_attr.attr;
	} else
		pr_err("%s: no space left on attr_pool\n", __func__);
#endif
...
#if defined(CONFIG_SEC_PM) && defined(CONFIG_MUIC_NOTIFIER)
struct notifier_block cpupm_muic_nb;
...
static int __init exynos_cpupm_muic_notifier_init(void)
{
	...
	return muic_notifier_register(&cpupm_muic_nb,
