...
static int acpm_ipc_wtime[MAX_LAZY_IPC] = {0, };
...
static int get_ipc_wtime(int idx)
{
	if (idx >= MAX_LAZY_IPC || acpm_ipc_wtime[idx] <= 100)
		return 100;
	if (acpm_ipc_wtime[idx] > 1500)
		return 1500;

	return acpm_ipc_wtime[idx];
}

static void calculate_ipc_time(unsigned int idx, unsigned int exec_time)
{
	if (idx >= MAX_LAZY_IPC)
		return;

	if (acpm_ipc_wtime[idx] < 100) {
		acpm_ipc_wtime[idx] = 100;
		return;
	}

	if (exec_time > acpm_ipc_wtime[idx] + 200)
		acpm_ipc_wtime[idx] += 10;
	else
		acpm_ipc_wtime[idx] -= 10;

	if (acpm_ipc_wtime[idx] > 1500)
		acpm_ipc_wtime[idx] = 1500;
}
...
int __acpm_ipc_send_data(unsigned int channel_id, struct ipc_config *cfg, bool w_mode)
{
	...
	unsigned int rear;
	unsigned int tmp_index;
	unsigned int wtime, ipc_idx, delta;
	...
	if (channel->polling && cfg->response) {
		if (w_mode)
			wtime = get_ipc_wtime(ipc_idx);
		else
			wtime = 100;
		start = sched_clock();
retry:
		timeout = start + IPC_TIMEOUT;
		timeout_flag = false;
		...
		if (w_mode) {
			delta = (sched_clock() - start)/1000;
			calculate_ipc_time(ipc_idx, delta);
		}
...
static void log_buffer_init(struct device *dev, struct device_node *node)
{
	...
	dbg_snapshot_add_bl_item_info("acpm_dram", virt_to_phys(acpm_debug->dump_base),
