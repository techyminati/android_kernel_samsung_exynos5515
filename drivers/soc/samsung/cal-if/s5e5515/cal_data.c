...
void s5e5515_lpd_sram_power_control(unsigned int enable)
{
	...
	if (enable) {
		ret = pmucal_rae_handle_seq(lpd_sram_on, lpd_sram_on_size);
	} else {
		ret = pmucal_rae_handle_seq(lpd_sram_off, lpd_sram_off_size);
	}

	if (ret) {
		pr_err("%s %s: lpd sram power control %s fail...\n",
				PMUCAL_PREFIX, __func__, enable ? "enable" : "disable");
	}
}
