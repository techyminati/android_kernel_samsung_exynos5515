...
static struct device *s2mpw03_pmic_sysfs;
static u8 pmic_rev;
static u8 pmic_ver;
#endif 
...
static struct mfd_cell s2mpw03_devs[] = {
	...
	{ .name = "s2mpw03-rtc", },
#ifdef	CONFIG_KEYBOARD_S2MPW03
	{ .name = "s2mpw03-power-keys", },
#endif
#ifdef	CONFIG_CHARGER_S2MPW03
	{ .name = "s2mpw03-charger", },
#endif
#ifdef CONFIG_FUELGAUGE_S2MPW03
...
#if 1
int s2mpw03_read_codec_reg(struct i2c_client *i2c, u8 reg, u8 *dest)
{
	int ret;
...
EXPORT_SYMBOL_GPL(s2mpw03_read_codec_reg);
#endif
				...
				const struct i2c_device_id *dev_id)
{
	...
	s2mpw03->pmic = i2c_new_dummy(i2c->adapter, I2C_ADDR_PMIC);
	s2mpw03->rtc = i2c_new_dummy(i2c->adapter, I2C_ADDR_RTC);
	s2mpw03->charger = i2c_new_dummy(i2c->adapter, I2C_ADDR_CHG);
	s2mpw03->fuelgauge = i2c_new_dummy(i2c->adapter, I2C_ADDR_FG);
	s2mpw03->close = i2c_new_dummy(i2c->adapter, I2C_ADDR_CLOSE);
...
#ifdef CONFIG_SEC_PM
	pmic_rev = s2mpw03->pmic_rev;
	pmic_ver = s2mpw03->pmic_ver;
#endif
...
#ifdef CONFIG_SEC_PM
static int s2mpw03_late_init(void) {
	int ret = 0;

	s2mpw03_pmic_sysfs = sec_device_create(NULL, "pmic");
...
late_initcall(s2mpw03_late_init);
#endif
