...
static int psy_set_property(const char *psy_name,
		enum power_supply_property prop, const union power_supply_propval *value)
{
...
static int otg_accessory_power(bool enable)
{
	...
	val.intval = enable;
	psy_set_property("otg", POWER_SUPPLY_PROP_ONLINE, &val);
#endif
	return 0;
}
#else
...
static int set_online(int event, int state)
{
	...
	psy_set_property("battery", POWER_SUPPLY_PROP_ONLINE, &val);
