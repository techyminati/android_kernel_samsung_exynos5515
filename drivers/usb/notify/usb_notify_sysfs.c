		...
		const char *buf, size_t size)
{
	...
	pr_info("%s req=%s now=%s\n", __func__, max_speed, max_speed_str[max_speed_idx]);
	ret = size;
error1:
	kfree(max_speed);
error:
		...
		const char *buf, size_t size)
{
	...
	if (size > MAX_WHITELIST_STR_LEN || size < 2) {
...
static DEVICE_ATTR(usb_maximum_speed, 0664, usb_maximum_speed_show, usb_maximum_speed_store);
