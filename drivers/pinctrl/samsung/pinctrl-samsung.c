...
u32 exynos_cmgp_eint_to_pin_num(int eint)
{
	struct samsung_pinctrl_drv_data *drvdata;
...
static int __init exynos5515_secgpio_get_nr_gpio(void)
{
	unsigned int i, j;
	int nr_gpio = 0;
	const struct samsung_pin_ctrl *pin_ctrl = s5e5515_of_data.ctrl;
	unsigned int num_ctrl = s5e5515_of_data.num_ctrl;
...
static struct gpio_dvs_t exynos5515_secgpio_dvs = {
	.result = &gpiomap_result,
	.check_gpio_status = check_gpio_status,
	.skip_grps = "gpb gpv", /* AUDIO, VTS */
};

const struct secgpio_dvs_data exynos5515_secgpio_dvs_data __initconst = {
	.gpio_dvs = &exynos5515_secgpio_dvs,
	.get_nr_gpio = exynos5515_secgpio_get_nr_gpio,
};
