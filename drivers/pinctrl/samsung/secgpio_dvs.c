...
static ssize_t checked_sleepGPIO_show(
	struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t check_requested_gpio_show(
	struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t check_requested_gpio_store(
	...
	struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = kstrtoint(buf, 10, &gdvs->gpio_num);
...
static const struct of_device_id secgpio_dvs_dt_match[] = {
	{ .compatible = "samsung,exynos5515-secgpio-dvs",
		.data = (void *)&exynos5515_secgpio_dvs_data },
	{ },
};
