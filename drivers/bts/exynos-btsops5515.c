...
static int set_drexbts(void __iomem *base, struct bts_stat *stat)
{
	...
	if (stat->drex_pf_on) {
		__raw_writel(stat->pf_token_control, base + PF_TOKEN_CONTROL);
		__raw_writel(stat->pf_token_threshold, base + PF_TOKEN_THRESHOLD0);
...
static int get_drexbts(void __iomem *base, struct bts_stat *stat)
{
	...
	if (stat->drex_pf_on) {
		stat->pf_token_control = __raw_readl(base + PF_TOKEN_CONTROL);
		stat->pf_token_threshold = __raw_readl(base + PF_TOKEN_THRESHOLD0);
