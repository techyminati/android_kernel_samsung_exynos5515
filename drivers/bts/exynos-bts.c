	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if (info[i].pd_on) {
			if (info[i].stat[ID_DEFAULT].drex_on) {
				...
				seq_printf(buf, " wdbuf_cutoff_con\t0x%.8X(NOT USED)\n",
						stat.wdbuf_cutoff_con);
				seq_printf(buf, " emergent_config_r/w\t0x%.8X\t0x%.8X\n\n",
						stat.emergent_config_r, stat.emergent_config_w);
			...
			if (info[i].stat[ID_DEFAULT].drex_pf_on) {
				...
				seq_printf(buf, "[%d] %s:\n pf_rreq_thrt_con\t0x%.8X(NOT USED)\n",
					i, info[i].name, stat.pf_rreq_thrt_con);
				seq_printf(buf, " allow_mo_for_region\t0x%.8X(NOT USED)\n",
						stat.allow_mo_for_region);
				seq_printf(buf, " pf_token_control\t0x%.8X\n", stat.pf_token_control);
				seq_printf(buf, " pf_token_threshold\t0x%.8X\n", stat.pf_token_threshold);
...
static int exynos_bts_write_config_open_show(struct seq_file *buf, void *d)
{
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		...
		if (info[i].pd_on) {
			...
			seq_printf(buf, "[%d] %s:   \n\twrite_config_0 0x%.8X\n\twrite_config_1 0x%.8X\n",
				i, info[i].name,
...
static int exynos_bts_emergent_config_open_show(struct seq_file *buf, void *d)
{
	struct bts_info *info = btsdev->bts_list;
	struct bts_stat stat;
	int ret, i = 0;
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		if (info[i].ops->get_emergent_config == NULL || !info[i].stat[ID_DEFAULT].drex_on)
			continue;

		spin_lock(&btsdev->lock);

		if (info[i].pd_on) {
			ret = info[i].ops->get_emergent_config(info[i].va_base, &stat);
			if (ret) {
				pr_err("%s: failed get emergnet_config\n", __func__);
				goto err_get_emergent_config;
			}
			seq_printf(buf, "[%d] %s:   \n\temergent_config_r 0x%.8X\n\temergent_config_w 0x%.8X\n",
					i, info[i].name,
					stat.emergent_config_r, stat.emergent_config_w);
		} else {
			seq_printf(buf, "[%d] %s:   \tLocal power off!\n",
...
static int exynos_bts_emergent_config_open(struct inode *inode, struct file *file)
{
	return single_open(file, exynos_bts_emergent_config_open_show, inode->i_private);
}

static ssize_t exynos_bts_emergent_config_write(struct file *file, const char __user *user_buf,
					size_t count, loff_t *ppos)
{
	char buf[64];
	ssize_t buf_size;
	...
	ret = sscanf(buf, "%d %d %x %x %x\n",
			&scen, &index, &arg0, &arg1);

	if (ret != 4) {
		pr_err("%s: sscanf failed. We need 4 inputs."\
			"<SCEN IP EMERGENT_CONFIG_R EMERGENT_CONFIG_W> count=(%d)\n",
			__func__, ret);
		return -EINVAL;
	}
	...
	if (info[index].ops->set_emergent_config != NULL &&
			info[index].stat[ID_DEFAULT].drex_on) {
		stat[scen].stat_on = true;
		stat[scen].drex_on = true;
		stat[scen].emergent_config_r = arg0;
		stat[scen].emergent_config_w = arg1;

		if (scen != btsdev->top_scen)
			goto out;

		if (info[index].pd_on) {
			if (info[index].ops->set_emergent_config(info[index].va_base, &stat[scen]))
				pr_warn("%s: set_emergent_config failed. input=(%d) err=(%d)\n",
						__func__, index, ret);
		}
	} else {
		pr_err("%s: Invalid index. [%d] is %s\n", __func__, index, info[index].name);
...
static int exynos_bts_pf_token_open_show(struct seq_file *buf, void *d)
{
	struct bts_info *info = btsdev->bts_list;
	struct bts_stat stat;
	int ret, i = 0;
	...
	for (i = 0; i < btsdev->num_bts; i++) {
		if (info[i].ops->get_pf_token == NULL ||
			!info[i].stat[ID_DEFAULT].drex_pf_on)
			continue;
		...
		if (info[i].pd_on) {
			ret = info[i].ops->get_pf_token(info[i].va_base, &stat);
			if (ret) {
				pr_err("%s: failed get pf_token\n", __func__);
				goto err_get_pf_token;
			}
			seq_printf(buf, "[%d] %s:   \n\tpf_token_control 0x%.8X\n\tpf_token_threshold 0x%.8X\n",
				i, info[i].name,
				stat.pf_token_control, stat.pf_token_threshold);
		} else {
			seq_printf(buf, "[%d] %s:   \tLocal power off!\n",
					i, info[i].name);
		}
...
static int exynos_bts_pf_token_open(struct inode *inode, struct file *file)
{
	return single_open(file, exynos_bts_pf_token_open_show, inode->i_private);
}

static ssize_t exynos_bts_pf_token_write(struct file *file, const char __user *user_buf,
					size_t count, loff_t *ppos)
{
	char buf[64];
	ssize_t buf_size;
	...
	if (ret != 4) {
		pr_err("%s: sscanf failed. We need 4 inputs."\
				"<SCEN IP PF_TOKEN_CONTROL PF_TOKEN_THRESHOLD> count=(%d)\n",
				__func__, ret);
		return -EINVAL;
	}
	...
	if (info[index].ops->set_pf_token != NULL &&
			info[index].stat[ID_DEFAULT].drex_pf_on) {
		stat[scen].stat_on = true;
		stat[scen].drex_pf_on = true;
		stat[scen].pf_token_control = arg0;
		stat[scen].pf_token_threshold = arg1;

		if (scen != btsdev->top_scen)
			goto out;

		if (info[index].pd_on) {
			if (info[index].ops->set_pf_token(info[index].va_base, &stat[scen]))
				pr_warn("%s: set_pf_token failed. input=(%d) err=(%d)\n",
						__func__, index, ret);
		}
	} else {
		pr_err("%s: Invalid index. [%d] is %s\n", __func__, index, info[index].name);
...
static const struct file_operations debug_bts_emergent_config_fops = {
	.open		= exynos_bts_emergent_config_open,
	.read		= seq_read,
	.write		= exynos_bts_emergent_config_write,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static const struct file_operations debug_bts_pf_token_fops = {
	.open		= exynos_bts_pf_token_open,
	.read		= seq_read,
	.write		= exynos_bts_pf_token_write,
	.llseek		= seq_lseek,
	.release	= single_release,
};
...
int exynos_bts_debugfs_init(void)
{
	...
	debugfs_create_file("emergent_config", 0440, den, NULL,
				&debug_bts_emergent_config_fops);

	debugfs_create_file("pf_token", 0440, den, NULL,
				&debug_bts_pf_token_fops);
...
static void bts_parse_setting(struct device_node *np, struct bts_stat *stat)
{
	...
	if (stat->stat_on) {
		...
		if (of_property_read_u32(np, "emergent_config_r",
					&(stat->emergent_config_r)))
			stat->emergent_config_r = EMERGENT_CONFIG_R_RESET;
		if (of_property_read_u32(np, "emergent_config_w",
					&(stat->emergent_config_w)))
			stat->emergent_config_w = EMERGENT_CONFIG_W_RESET;
		of_property_read_u32(np, "drex_pf_on", &tmp);
		stat->drex_pf_on = tmp ? true : false;

		if (of_property_read_u32(np, "pf_token_control",
					&(stat->pf_token_control)))
			stat->pf_token_control = PF_TOKEN_CONTROL_RESET;
		if (of_property_read_u32(np, "pf_token_threshold",
					&(stat->pf_token_threshold)))
			stat->pf_token_threshold = PF_TOKEN_THRESHOLD_RESET;
