...
#define LPD_METADATA_API_VERSION		_VERSION(0, 9, 3)
...
struct clock_hands {
	...
	struct lpd_image 		hand[LPD_CLOCK_HAND_MAX];
};
...
enum lpd_dpu_layers {
	LPD_DPU_LAYER_BACKGROUND 	= 0,
	LPD_DPU_LAYER_CANVAS,
	LPD_DPU_LAYER_MAX,
}__attribute__((packed));
...
struct lpd_image_info {
	...
	struct lpd_buffer_info image_info[HAND_COUNT*LPD_CLOCK_HAND_MAX + ICON_COUNT];
...
typedef struct lpd_metadata {
	...
	struct digit				digits[DIGIT_COUNT];
...
struct lpd_dpu_info {
	...
	__u32 lpd_dma_address[LPD_DPU_LAYER_MAX]; 					/* lpd_dma_address[0] is layer0 for dma0/dpp0/window1 */
												/* lpd_dma_address[1] is layer1 for dma1/dpp1/window0 */
	__u32 lpd_dma_size[LPD_DPU_LAYER_MAX];
