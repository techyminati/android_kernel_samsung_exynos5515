...
//	dev_set_socdata(&pdev->dev, "Exynos", "PWM");
	dev_err(&pdev->dev, "Device probe start\n");

	chip = devm_kzalloc(&pdev->dev, sizeof(*chip), GFP_KERNEL);
	if (chip == NULL) {
		dev_err(&pdev->dev, "Device prove fail: Nomem\n");
		return -ENOMEM;
	}
	...
	dev_err(&pdev->dev, "Device probe finished\n");

	return 0;

chip_add_err:
	dev_err(&pdev->dev, "chip add failed\n");
	clk_disable_unprepare(chip->pwm_sclk);
sclk_err:
	dev_err(&pdev->dev, "sclk add failed\n");
	clk_disable_unprepare(chip->pwm_pclk);
base_clk_err:
	dev_err(&pdev->dev, "pclk add failed\n");
	pwm_samsung_update_ip_idle_status(chip, 1);
	return ret;
