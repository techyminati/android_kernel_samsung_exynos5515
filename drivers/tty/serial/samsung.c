...
/* Allocate 4MB of buffer for UART logging */
#define LOG_BUFFER_SIZE		(0x400000)
				...
				unsigned char* trace_buf, int len)
{
	...
	if (dir == 4) {
		local_buf->index += scnprintf(local_buf->buffer + local_buf->index,
				local_buf->size - local_buf->index, "[PM] ");
		local_buf->index += scnprintf(local_buf->buffer + local_buf->index,
				local_buf->size - local_buf->index,
				"%s", trace_buf);
	} else if (dir == 3) {
...
static void s3c24xx_serial_start_tx(struct uart_port *port)
{
	...
	if (ourport->uart_logging && ourport->port.line == BLUETOOTH_UART_PORT_LINE) {
		snprintf(uart_log_buf, LOG_REG_DEBUG_LEN,"[%s: %s(%d)]PREPARE : Turn on BT wake_peer\n", __FILE__, __FUNCTION__, __LINE__);
		uart_copy_to_local_buf(4, &ourport->uart_local_buf, uart_log_buf, sizeof(uart_log_buf));
	}
#endif /* logging */
...
#ifdef TRACE_UART_SUSPEND_RESUME
			if (ourport->uart_logging && ourport->port.line == BLUETOOTH_UART_PORT_LINE) {
				snprintf(uart_log_buf, LOG_REG_DEBUG_LEN,"[%s: %s(%d)]PREPARE : Turn off BT wake_peer\n", __FILE__, __FUNCTION__, __LINE__);
				uart_copy_to_local_buf(4, &ourport->uart_local_buf, uart_log_buf, sizeof(uart_log_buf));
			}
#endif /* logging */
				...
				unsigned long cmd, void *v)
{
	...
	case SICD_AUD_ENTER:
		list_for_each_entry(ourport, &drvdata_list, node) {
			...
			if (ourport->uart_logging && ourport->port.line == BLUETOOTH_UART_PORT_LINE) {
				snprintf(uart_log_buf, LOG_REG_DEBUG_LEN,"[%s: %s(%d)] %s\n", __FILE__, __FUNCTION__, __LINE__,
						(cmd == SICD_ENTER)?"SICD_ENTER":(cmd == SICD_AUD_ENTER)?"SICD_AUD_ENTER":"Unknown");
				uart_copy_to_local_buf(4, &ourport->uart_local_buf, uart_log_buf, sizeof(uart_log_buf));
			}
#endif
		}
...
#ifdef TRACE_UART_SUSPEND_RESUME
		if (ourport->uart_logging && ourport->port.line == BLUETOOTH_UART_PORT_LINE) {
			snprintf(uart_log_buf, LOG_REG_DEBUG_LEN,"[%s: %s(%d)]\n", __FILE__, __FUNCTION__, __LINE__);
			uart_copy_to_local_buf(4, &ourport->uart_local_buf, uart_log_buf, sizeof(uart_log_buf));
		}
#endif /* logging */
	}
