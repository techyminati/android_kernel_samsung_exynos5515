...
{
#if defined(DHD_SSW_P2P_USE_PERM_ADDRESS)
...
wl_set_set_cipher(struct net_device *dev, struct cfg80211_connect_params *sme)
{
...
#if defined(WL_SAE) || defined(BCMSUP_4WAY_HANDSHAKE)
	...
	if (((cfg->wdev->wiphy->features & NL80211_FEATURE_FW_4WAY_HANDSHAKE) &&
		...
		cfg->wdev->wiphy, NL80211_EXT_FEATURE_4WAY_HANDSHAKE_STA_PSK) &&
		 (FW_SUPPORTED(dhdp, sae)))) {
...
#endif /* WL_SAE ||  BCMSUP_4WAY_HANDSHAKE */
	...
	bcm_struct_cfgdev *cfgdev, const wl_event_msg_t *e, void *data)
{
	...
	cfg->ext_auth_channel = auth_req->channel;
...
#endif /* WL_CFG80211_P2P_DEV_IF */
		WL_INFORM_MEM(("started listen on channel:%d dur:%d cookie:%llu\n",
 ...
 */
			...
			/* update channel */
#ifdef WL_SAE
			if (cfg->ext_auth_channel) {
				mf_params->channel = cfg->ext_auth_channel;
				WL_INFORM(("Channel %d \n", mf_params->channel));
			} else {
#endif // endif
				if (!channel) {
...
wl_flush_fw_log_buffer(struct net_device *dev, uint32 logset_mask)
{
	...
	if (dhd->memdump_type == DUMP_TYPE_DONGLE_TRAP
		|| dhd->memdump_type == DUMP_TYPE_RESUMED_ON_TIMEOUT
		|| dhd->memdump_type == DUMP_TYPE_RESUMED_ON_TIMEOUT_RX
		|| dhd->memdump_type == DUMP_TYPE_RESUMED_ON_TIMEOUT_TX)
		return;
#endif // endif
