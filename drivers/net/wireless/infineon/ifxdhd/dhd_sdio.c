...
do { \
	...
	} while (ifxsdh_regfail(bus->sdh) && (++retryvar <= retry_limit)); \
	if (retryvar) { \
...
#define PKT_AVAILABLE(bus, intstatus)	(ifxsdh_get_dstatus(bus->sdh) & STATUS_F2_PKT_AVAILABLE)

#define HOSTINTMASK		(I_HMB_FC_CHANGE | I_HMB_HOST_INT)

#define GSPI_PR55150_BAILOUT									\
do {												\
	uint32 dstatussw = ifxsdh_get_dstatus((void *)bus->sdh);				\
	uint32 dstatushw = ifxsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0, SPID_STATUS_REG, NULL);	\
...
static int dhd_ifxsdh_recv_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags,
	uint8 *buf, uint nbytes,
	void *pkt, bcmsdh_cmplt_fn_t complete, void *handle);
static int dhd_ifxsdh_send_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags,
	uint8 *buf, uint nbytes,
...
static int dhd_ifxsdh_send_buffer(void *bus, uint8 *frame, uint16 len);
...
dhdsdio_tune_fifoparam(struct dhd_bus *bus)
{
	...
	/* Update watermark */
	if (wm > 0) {
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_WATERMARK, wm, &err);

		devctl = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
		devctl |= SBSDIO_DEVCTL_F2WM_ENAB;
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, &err);
	}

	/* Update MES */
	if (mes > 0) {
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_MESBUSYCTRL,
			(mes | SBSDIO_MESBUSYCTRL_ENAB), &err);
	}

	DHD_INFO(("Apply overflow WAR: 0x%02x 0x%02x 0x%02x\n",
		ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err),
		ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_WATERMARK, &err),
		ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_MESBUSYCTRL, &err)));
}
...
{
	int err = 0;
	ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SBADDRLOW,
	                 (address >> 8) & SBSDIO_SBADDRLOW_MASK, &err);
	if (!err)
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SBADDRMID,
		                 (address >> 16) & SBSDIO_SBADDRMID_MASK, &err);
	if (!err)
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SBADDRHIGH,
...
dhdsdio_wkwlan(dhd_bus_t *bus, bool on)
{
	...
	if (bus->sih->buscoretype == SDIOD_CORE_ID) {
		...
		regdata = ifxsdh_cfg_read_word(sdh, SDIO_FUNC_0, SPID_CONFIG, NULL);
		...
		ifxsdh_cfg_write_word(sdh, SDIO_FUNC_0, SPID_CONFIG, regdata, &err);
	}
}
#endif /* BCMSPI */
...
dhdsdio_oobwakeup_init(dhd_bus_t *bus)
{
	...
	ifxsdh_reg_write(bus->sdh, addr, 4, 2);
	val = ifxsdh_reg_read(bus->sdh, data, 4);
	val |= CC_CHIPCTRL2_GPIO1_WAKEUP;
	ifxsdh_reg_write(bus->sdh, data, 4, val);
...
dhdsdio_sr_cap(dhd_bus_t *bus)
{
	...
	if (bus->sih->chip == BCM43430_CHIP_ID ||
		...
		if (ifxsdh_reg_read(bus->sdh, addr, 4) != 0)
			cap = TRUE;
	...
	if (
		...
		core_capext = TRUE;
			...
			core_capext = ifxsdh_reg_read(bus->sdh,
	...
	if ((bus->sih->chip == BCM4335_CHIP_ID) ||
		...
		addr = SI_ENUM_BASE(bus->sih) + OFFSETOF(chipcregs_t, chipcontrol_addr);
		data = SI_ENUM_BASE(bus->sih) + OFFSETOF(chipcregs_t, chipcontrol_data);
		ifxsdh_reg_write(bus->sdh, addr, 4, CC_PMUCC3);
		enabval = ifxsdh_reg_read(bus->sdh, data, 4);
...
dhdsdio_sr_init(dhd_bus_t *bus)
{
	...
	if (bus->sih->chip == BCM43012_CHIP_ID ||
	    ...
	    bus->sih->chip == CYW55560_CHIP_ID) {
		val = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_WAKEUPCTRL, NULL);
		val |= 1 << SBSDIO_FUNC1_WCTRL_ALPWAIT_SHIFT;
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_WAKEUPCTRL,
	...
	if (bus->sih->chip == BCM43012_CHIP_ID ||
	    bus->sih->chip == CYW55500_CHIP_ID ||
	    bus->sih->chip == CYW55560_CHIP_ID) {
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1,
...
dhdsdio_clk_kso_init(dhd_bus_t *bus)
{
	 ...
	 */
	val = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, NULL);
	if (!(val & SBSDIO_FUNC1_SLEEPCSR_KSO_MASK)) {
		val |= (SBSDIO_FUNC1_SLEEPCSR_KSO_EN << SBSDIO_FUNC1_SLEEPCSR_KSO_SHIFT);
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, val, &err);
		if (err)
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, wr_val, &err);
	...
	do {
		rd_val = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, &err);
...
dhdsdio_clk_kso_iovar(dhd_bus_t *bus, bool on)
{
	...
	if (on == FALSE) {
		...
		DHD_ERROR(("%s: KSO disable clk: 0x%x\n", __FUNCTION__,
			ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
...
dhdsdio_sleepcsr_get(dhd_bus_t *bus)
{
	...
	val = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SLEEPCSR, &err);
	if (err)
...
{
	return ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_BRCM_CARDCAP, NULL);
}
...
dhdsdio_devcap_set(dhd_bus_t *bus, uint8 cap)
{
	...
	ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_BRCM_CARDCAP, cap, &err);
	if (err)
...
dhdsdio_clk_devsleep_iovar(dhd_bus_t *bus, bool on)
{
	...
	if (on == TRUE) {
		...
		if (!SLPAUTO_ENAB(bus))
			...
			val = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, &err);
			if ((val & SBSDIO_CSR_MASK) == 0) {
				DHD_ERROR(("%s: No clock before enter sleep:0x%x\n",
					__FUNCTION__, val));

				/* Reset clock request */
				ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR,
		...
		if (err == 0) {
			...
			SPINWAIT_SLEEP(sdioh_spinwait_sleep,
				(((csr = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1,
...
#define HT_AVAIL_ERROR_MAX 10
	...
	if (on) {
		...
		ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, clkreq, &err);
		if (err) {
			ht_avail_error++;
		...
		clkctl = ifxsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, &err);
		if (err) {
		...
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only) && pendok) {
			...
			devctl = ifxsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, &err);
			if (err) {
			...
			ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, devctl, &err);
		...
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only)) {
			SPINWAIT_SLEEP(sdioh_spinwait_sleep,
				((clkctl = ifxsdh_cfg_read(sdh, SDIO_FUNC_1,
			                                    SBSDIO_FUNC1_CHIPCLKCSR, &err)),
...
dhdsdio_set_sdmode(dhd_bus_t *bus, int32 sd_mode)
{
	...
	err = ifxsdh_iovar_op(bus->sdh, "sd_mode", NULL, 0,
...
dhdsdio_sdclk(dhd_bus_t *bus, bool on)
{
	...
	if (on) {
		if (bus->idleclock == DHD_IDLE_STOP) {
			...
			iovalue = 1;
			err = ifxsdh_iovar_op(bus->sdh, "sd_clock", NULL, 0,
			...
			err = ifxsdh_iovar_op(bus->sdh, "sd_divisor", NULL, 0,
			                      &iovalue, sizeof(iovalue), TRUE);
			if (err) {
...
dhdsdio_bussleep(dhd_bus_t *bus, bool sleep)
{
	...
	if (sleep) {
		...
		if (!SLPAUTO_ENAB(bus)) {
			...
			ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR,
				SBSDIO_FORCE_HW_CLKREQ_OFF, NULL);

			/* Isolate the bus */
			ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL,
		...
		err = ifxsdh_iovar_op(bus->sdh, "sd3_tuning_disable", NULL, 0,
		...
		if (!SLPAUTO_ENAB(bus)) {
			ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, 0, &err);

			/* Force pad isolation off if possible (in case power never toggled) */
			ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_DEVICE_CTL, 0, NULL);
...
static int dhdsdio_txpkt(dhd_bus_t *bus, uint chan, void** pkts, int num_pkt, bool free_pkt)
{
	...
	/* dhd_ifxsdh_send_buf ignores the buffer pointer if he packet
	...
	ret = dhd_ifxsdh_send_buf(bus, ifxsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
...
#endif // endif
		ret = dhd_ifxsdh_send_buffer(bus, frame, len);
	}
	bus->ctrl_frame_stat = FALSE;
#ifdef DHD_ULP
...
dhd_bus_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf, "dpc_sched %d host interrupt%spending\n",
	            bus->dpc_sched, (ifxsdh_intr_pending(bus->sdh) ? " " : " not "));
...
dhdsdio_membytes(dhd_bus_t *bus, bool write, uint32 address, uint8 *data, uint size)
{
	...
	while (size) {
		...
		if ((bcmerror = ifxsdh_rwdata(bus->sdh, write, sdaddr, data, dsize))) {
...
xfer_done:
	...
	if (dhdsdio_set_siaddr_window(bus, ifxsdh_cur_sbwad(bus->sdh))) {
		DHD_ERROR(("%s: FAILED to set window back to 0x%x\n", __FUNCTION__,
			ifxsdh_cur_sbwad(bus->sdh)));
	}
...
dhd_serialconsole(dhd_bus_t *bus, bool set, bool enable, int *bcmerror)
{
	...
	*bcmerror = 0;
	...
	ifxsdh_reg_write(bus->sdh, addr, 4, 1);
	if (ifxsdh_regfail(bus->sdh)) {
		*bcmerror = BCME_SDIO_ERROR;
		return -1;
	}
	int_val = ifxsdh_reg_read(bus->sdh, data, 4);
	if (ifxsdh_regfail(bus->sdh)) {
		*bcmerror = BCME_SDIO_ERROR;
		return -1;
	}
	...
	if (enable)
		...
		int_val &= ~uart_enab;
	ifxsdh_reg_write(bus->sdh, data, 4, int_val);
	if (ifxsdh_regfail(bus->sdh)) {
		*bcmerror = BCME_SDIO_ERROR;
		return -1;
	}
                ...
                void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_GVAL(IOV_SDREG):
	{
		...
		size = sd_ptr->func;
		int_val = (int32)ifxsdh_reg_read(bus->sdh, addr, size);
		if (ifxsdh_regfail(bus->sdh))
			bcmerror = BCME_SDIO_ERROR;
	...
	case IOV_SVAL(IOV_SDREG):
	{
		...
		addr = ((uintptr)bus->regs + sd_ptr->offset);
		size = sd_ptr->func;
		ifxsdh_reg_write(bus->sdh, addr, size, sd_ptr->value);
		if (ifxsdh_regfail(bus->sdh))
			bcmerror = BCME_SDIO_ERROR;
		break;
	}
	...
	case IOV_SVAL(IOV_SBREG):
	{
		...
		addr = SI_ENUM_BASE(bus->sih) + sdreg.offset;
		size = sdreg.func;
		ifxsdh_reg_write(bus->sdh, addr, size, sdreg.value);
		if (ifxsdh_regfail(bus->sdh))
			bcmerror = BCME_SDIO_ERROR;
		break;
	}
	...
	case IOV_GVAL(IOV_SDCIS):
	{
		*(char *)arg = 0;
		...
		bcmstrcat(arg, "\nFunc 0\n");
		ifxsdh_cis_read(bus->sdh, 0x10, (uint8 *)arg + strlen(arg), SBSDIO_CIS_SIZE_LIMIT);
		bcmstrcat(arg, "\nFunc 1\n");
		ifxsdh_cis_read(bus->sdh, 0x11, (uint8 *)arg + strlen(arg), SBSDIO_CIS_SIZE_LIMIT);
		bcmstrcat(arg, "\nFunc 2\n");
		ifxsdh_cis_read(bus->sdh, 0x12, (uint8 *)arg + strlen(arg), SBSDIO_CIS_SIZE_LIMIT);
		break;
	}
	...
	case IOV_SVAL(IOV_WATERMARK):
		...
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_WATERMARK, (uint8)watermark, NULL);
		break;
...
dhdsdio_cfg_write_buffer(dhd_bus_t *bus, uint32 *buffer, uint offset)
{
	...
	ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, offset, ((*buffer) >> 0)
		 & CFG_WRITE_BYTE_MASK, &err);
	if (!err) {
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, offset+1, ((*buffer) >> 8)
			& CFG_WRITE_BYTE_MASK, &err);
	}
	if (!err) {
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, offset+2, ((*buffer) >> 16)
			& CFG_WRITE_BYTE_MASK, &err);
	}
	if (!err) {
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, offset+3, ((*buffer) >> 24)
			& CFG_WRITE_BYTE_MASK, &err);
	}
...
{
	*buffer = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, offset+0, NULL) << 0;
	*buffer = *buffer | ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, offset+1, NULL) << 8;
	*buffer = *buffer | ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, offset+2, NULL) << 16;
	*buffer = *buffer | ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, offset+3, NULL) << 24;
                 ...
                 void *params, int plen, void *arg, int len, bool set)
{
	...
	if ((vi = bcm_iovar_lookup(dhdsdio_iovars, name)) == NULL) {
		...
		bcmerror = ifxsdh_iovar_op(bus->sdh, name, params, plen, arg, len, set);

		/* Check for bus configuration changes of interest */

		/* If it was divisor change, read the new one */
		if (set && strcmp(name, "sd_divisor") == 0) {
			if (ifxsdh_iovar_op(bus->sdh, "sd_divisor", NULL, 0,
		...
		if (set && strcmp(name, "sd_mode") == 0) {
			if (ifxsdh_iovar_op(bus->sdh, "sd_mode", NULL, 0,
		...
		if (set && strcmp(name, "sd_blocksize") == 0) {
			int32 fnum = 2;
			if (ifxsdh_iovar_op(bus->sdh, "sd_blocksize", &fnum, sizeof(int32),
...
dhd_bus_stop(struct dhd_bus *bus, bool enforce_mutex)
{
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || bus->dhd->hang_was_sent) {
		...
		saveclk = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, &err);
		if (!err) {
			ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR,
		...
		ifxsdh_intr_disable(bus->sdh);
#ifndef BCMSPI
		ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_IOEN, SDIO_FUNC_ENABLE_1, NULL);
#endif /* !BCMSPI */
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	while (!enable && !dhd_timeout_expired(&tmo)) {
		dstatus = ifxsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0, SPID_STATUS_REG, NULL);
	...
	ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_IOEN, enable, NULL);
	...
	while (ready != enable && !dhd_timeout_expired(&tmo))
	        ready = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_0, SDIOD_CCCR_IORDY, NULL);
	...
	if (ready == enable) {
		...
		if (bus->sih->buscorerev < 15) {
			ifxsdh_cfg_write(bus->sdh, SDIO_FUNC_1, SBSDIO_WATERMARK,
				(uint8)watermark, &err);
		}
...
dhdsdio_rxfail(dhd_bus_t *bus, bool abort, bool rtx)
{
	...
	if (abort) {
		ifxsdh_abort(sdh, SDIO_FUNC_2);
	}

	ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_FRAMECTRL, SFC_RF_TERM, &err);
	if (err) {
	...
	for (lastrbc = retries = 0xffff; retries > 0; retries--) {
		hi = ifxsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_RFRAMEBCHI, NULL);
		lo = ifxsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_RFRAMEBCLO, &err);
		if (err) {
...
dhdsdio_read_control(dhd_bus_t *bus, uint8 *hdr, uint len, uint doff)
{
	...
	sdret = dhd_ifxsdh_recv_buf(bus, ifxsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
...
dhdsdio_rxglom(dhd_bus_t *bus, uint8 rxseq)
{
	...
	if (bus->glom) {
		 ...
		 */
		if (usechain) {
			errcode = dhd_ifxsdh_recv_buf(bus,
			                              ifxsdh_cur_sbwad(bus->sdh), SDIO_FUNC_2,
...
dhdsdio_readframes(dhd_bus_t *bus, uint maxframes, bool *finished)
{
	...
	*finished = FALSE;
	...
	if (bus->bus == SPI_BUS) {
		...
		dstatus = ifxsdh_get_dstatus(bus->sdh);
		if (dstatus == 0)
		...
		DHD_TRACE(("Device status from bit-reconstruction = 0x%x\n",
		          ifxsdh_get_dstatus((void *)bus->sdh)));
		...
		if (dhd_readahead && bus->nextlen) {
			...
			if (!(pkt = PKTGET(osh, rdlen + DHD_SDALIGN, FALSE))) {
				if (bus->bus == SPI_BUS) {
					...
					sdret = dhd_ifxsdh_recv_buf(bus,
					                            ifxsdh_cur_sbwad(sdh),
					                            SDIO_FUNC_2,
					                            F2SYNC, rxbuf, rdlen,
					                            NULL, NULL, NULL);
					bus->f2rxdata++;
...
#ifdef BCMSPI
					if (ifxsdh_get_dstatus((void *)bus->sdh) &
					                STATUS_UNDERFLOW) {
						bus->nextlen = 0;
						*finished = TRUE;
				...
				/* Read the entire frame */
				sdret = dhd_ifxsdh_recv_buf(bus, ifxsdh_cur_sbwad(sdh),
				                            SDIO_FUNC_2,
				                            F2SYNC, rxbuf, rdlen,
				                            pkt, NULL, NULL);
				bus->f2rxdata++;
				ASSERT(sdret != BCME_PENDING);
#ifdef BCMSPI
				if (ifxsdh_get_dstatus((void *)bus->sdh) & STATUS_UNDERFLOW) {
					bus->nextlen = 0;
					*finished = TRUE;
			...
			if (bus->bus == SPI_BUS) {
				...
				dstatus = ifxsdh_get_dstatus((void *)bus->sdh);
...
clkwait:
	 ...
	 */
	if (bus->intr && bus->intdis && !ifxsdh_regfail(sdh)) {
	...
	if ((bus->dhd->busstate == DHD_BUS_DOWN) || ifxsdh_regfail(sdh)) {
...
dhd_bus_watchdog(dhd_pub_t *dhdp)
{
	...
	if (!SLPAUTO_ENAB(bus) && (bus->poll && (++bus->polltick >= bus->pollrate))) {
		...
		if (!bus->intr || (bus->intrcount == bus->lastintrs)) {
			...
			if (!bus->dpc_sched) {
				uint8 devpend;
				devpend = ifxsdh_cfg_read(bus->sdh, SDIO_FUNC_0,
			...
			if (!bus->dpc_sched) {
				uint32 devpend;
				devpend = ifxsdh_cfg_read_word(bus->sdh, SDIO_FUNC_0,
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	if (bus->intr) {
		...
		ifxsdh_intr_disable(sdh);
		if ((ret = ifxsdh_intr_reg(sdh, dhdsdio_isr, bus)) != 0) {
			DHD_ERROR(("%s: FAILED: ifxsdh_intr_reg returned %d\n",
			           __FUNCTION__, ret));
			goto fail;
		}
	...
	ifxsdh_reg_write(bus->sdh, 0x18000620, 2, 11);
	ifxsdh_reg_write(bus->sdh, 0x18000628, 4, 0x00F80001);
                     ...
                     uint16 devid)
{
	...
	DHD_ERROR(("F1 signature read @0x18000000=0x%4x\n",
		ifxsdh_reg_read(bus->sdh, si_enum_base(devid), 4)));
	...
	ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, DHD_INIT_CLKCTL1, &err);
	if (!err)
		clkctl = ifxsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, &err);
	...
	if (err || ((clkctl & ~SBSDIO_AVBITS) != DHD_INIT_CLKCTL1)) {
...
#endif /* !BCMSPI */
#ifndef BCMSPI
		numfn = ifxsdh_query_iofnum(sdh);
		ASSERT(numfn <= SDIOD_MAX_IOFUNCS);

		/* Make sure ALP is available before trying to read CIS */
		SPINWAIT(((clkctl = ifxsdh_cfg_read(sdh, SDIO_FUNC_1,
		...
		for (fn = 0; fn <= numfn; fn++) {
			...
			if ((err = ifxsdh_cis_read(sdh, fn, cis[fn],
		...
		*/
			...
			if (ifxsdh_iovar_op(sdh, "sd_blocksize", &fn, sizeof(int32),
			...
			value = fn << 16 | value;
			if (ifxsdh_iovar_op(sdh, "sd_blocksize", NULL, 0, &value,
			...
			if (fn == F2_BLOCK) {
				...
				if (ifxsdh_iovar_op(sdh, "sd_blocksize",
	...
	ifxsdh_chipinfo(sdh, bus->sih->chip, bus->sih->chiprev);
...
dhdsdio_probe_init(dhd_bus_t *bus, osl_t *osh, void *sdh)
{
	...
	ifxsdh_cfg_write(sdh, SDIO_FUNC_0, SDIOD_CCCR_IOEN, SDIO_FUNC_ENABLE_1, NULL);
#endif /* !BCMSPI */
	...
	ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, 0, NULL);
#endif /* !BCMSPI */
	...
	if (ifxsdh_iovar_op(sdh, "sd_divisor", NULL, 0,
	                    &bus->sd_divisor, sizeof(int32), FALSE) != BCME_OK) {
	...
	if (ifxsdh_iovar_op(sdh, "sd_mode", NULL, 0,
	                    &bus->sd_mode, sizeof(int32), FALSE) != BCME_OK) {
	...
	fnum = 2;
	if (ifxsdh_iovar_op(sdh, "sd_blocksize", &fnum, sizeof(int32),
	                    &bus->blocksize, sizeof(int32), FALSE) != BCME_OK) {
		bus->blocksize = 0;
		DHD_ERROR(("%s: fail on fn %d %s get\n", __FUNCTION__, fnum, "sd_blocksize"));
	} else {
	...
	if (ifxsdh_iovar_op(sdh, "sd_rxchain", NULL, 0,
...
dhd_bus_register(void)
{
	...
	return ifxsdh_register(&dhd_sdio);
}
...
static int
dhd_ifxsdh_recv_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags, uint8 *buf, uint nbytes,
	void *pkt, bcmsdh_cmplt_fn_t complete_fn, void *handle)
{
	...
	status = ifxsdh_recv_buf(bus->sdh, addr, fn, flags, buf, nbytes, pkt, complete_fn, handle);

	return status;
}

static int
dhd_ifxsdh_send_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags, uint8 *buf, uint nbytes,
	void *pkt, bcmsdh_cmplt_fn_t complete_fn, void *handle, int max_retry)
{
	...
	sdh = bus->sdh;
	do {
		ret = ifxsdh_send_buf(bus->sdh, addr, fn, flags, buf, nbytes,
		...
		if (ret == BCME_NODEVICE) {
			...
			bus->dhd->tx_errors++;
			ifxsdh_abort(sdh, SDIO_FUNC_2);
			ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_FRAMECTRL,
				SFC_WF_TERM, NULL);
			for (i = 0; i < READ_FRM_CNT_RETRIES; i++) {
				uint8 hi, lo;
				hi = ifxsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_WFRAMEBCHI,
					NULL);
				lo = ifxsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_WFRAMEBCLO,
					NULL);
				bus->f1regdata += 2;
				if ((hi == 0) && (lo == 0))
					break;
...
dhd_bus_is_ioready(struct dhd_bus *bus)
{
	...
	enable = (SDIO_FUNC_ENABLE_1 | SDIO_FUNC_ENABLE_2);
	sdh = bus->sdh;
	return (enable == ifxsdh_cfg_read(sdh, SDIO_FUNC_0, SDIOD_CCCR_IORDY, NULL));
}
...
dhd_sdio_reg_read(struct dhd_bus *bus, ulong addr)
{
	...
	rval = ifxsdh_reg_read(bus->sdh, addr, 4);
...
dhd_sdio_reg_write(struct dhd_bus *bus, ulong addr, uint32 val)
{
	...
	ifxsdh_reg_write(bus->sdh, addr, 4, val);
...
uint8 dhd_bus_cfg_read(void *h, uint fun_num, uint32 addr, int *err)
{
	...
	intrd = ifxsdh_cfg_read(bus->sdh, fun_num, addr, err);
...
void dhd_bus_cfg_write(void *h, uint fun_num, uint32 addr, uint8 val, int *err)
{
	...
	ifxsdh_cfg_write(bus->sdh, fun_num, addr, val, err);
...
_dhdsdio_download_btfw(struct dhd_bus *bus)
{
	...
	ifxsdh_reg_write(bus->sdh, bt2wlan_pwrup_adr, 4, BT2WLAN_PWRUP_WAKE);
	/*
	...
	while (read_more_btbytes (bus, image, line, &addr_mode, &hiAddress, &dest_addr,
		...
		if (!ISALIGNED(start_addr, 4)) {
			...
			start_data = ifxsdh_reg_read(bus->sdh, start_addr, 4);
		...
		if (!ISALIGNED(end_addr, 4)) {
			...
			end_data = ifxsdh_reg_read(bus->sdh, end_addr, 4);
...
static int
dhd_ifxsdh_send_buffer(void *bus, uint8 *frame, uint16 len)
{
	int ret = -1;

	ret = dhd_ifxsdh_send_buf(bus, ifxsdh_cur_sbwad(((dhd_bus_t*)bus)->sdh),
...
dhd_bus_set_default_min_res_mask(struct dhd_bus *bus)
{
	...
	switch (bus->sih->chip) {
	case BCM4339_CHIP_ID:
		ifxsdh_reg_write(bus->sdh, SI_ENUM_BASE(bus->sih) + 0x618, 4, 0x3fcaf377);
		if (ifxsdh_regfail(bus->sdh)) {
...
dhd_bus_pmu_reg_reset(dhd_pub_t *dhdp)
{
	...
	ifxsdh_reg_write(bus->sdh, si_get_pmu_reg_addr(bus->sih,
...
dhd_bus_readwrite_bp_addr(dhd_pub_t *dhdp, uint addr, uint size, uint* data, bool read)
{
	...
	if (read) {
		*data = (int32)ifxsdh_reg_read(bus->sdh, addr, size);
	} else {
		ifxsdh_reg_write(bus->sdh, addr, size, *data);
	}
