...
extern struct resource ifx_wlan_resources;
extern struct wifi_platform_data ifx_wlan_control;
#else
...
#endif // endif
struct resource ifx_wlan_resources = {0};
struct wifi_platform_data ifx_wlan_control = {0};
#if defined(STRICT_GCC_WARNINGS) && defined(__GNUC__)
...
static int wifi_ctrlfunc_register_drv(void)
{
	...
	if (dts_enabled) {
		struct resource *resource;
		adapter->wifi_plat_data = (void *)&ifx_wlan_control;
		resource = &ifx_wlan_resources;
		adapter->irq_num = resource->start;
		adapter->intr_flags = resource->flags & IRQF_TRIGGER_MASK;
#ifdef DHD_ISR_NO_SUSPEND
		adapter->intr_flags |= IRQF_NO_SUSPEND;
