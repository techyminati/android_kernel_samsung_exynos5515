...
#define DHD_ALLOC_HUGESIZE

#if defined(DHD_ALLOC_HUGESIZE)
#define KMALLOC(a, b) kvmalloc(a, b)
#define KFREE(a) kvfree(a)
#else
...
void
*ifx_wlan_mem_prealloc(int section, unsigned long size)
{
...
EXPORT_SYMBOL(ifx_wlan_mem_prealloc);

int
ifx_init_wlan_mem(void)
{
	int i;
	int j;
...
EXPORT_SYMBOL(ifx_init_wlan_mem);

void
ifx_exit_wlan_mem(void)
{
	int i = 0;
...
EXPORT_SYMBOL(ifx_exit_wlan_mem);
