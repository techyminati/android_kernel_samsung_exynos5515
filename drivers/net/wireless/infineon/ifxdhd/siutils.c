...
si_buscore_prep(si_info_t *sii, uint bustype, uint devid, void *sdh)
{
	...
	if (BUSTYPE(bustype) == SDIO_BUS) {
		...
		ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, clkset, &err);
		if (!err) {
			uint8 clkval;

			/* If register supported, wait for ALPAvail and then force ALP */
			clkval = ifxsdh_cfg_read(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_CHIPCLKCSR, NULL);
			if ((clkval & ~SBSDIO_AVBITS) == clkset) {
				SPINWAIT(((clkval = ifxsdh_cfg_read(sdh, SDIO_FUNC_1,
		...
		ifxsdh_cfg_write(sdh, SDIO_FUNC_1, SBSDIO_FUNC1_SDIOPULLUP, 0, NULL);
	}
