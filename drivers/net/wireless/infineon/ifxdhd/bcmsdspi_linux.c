...
bcmsdh_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (sd_pci_slot != 0xFFFFFFFF) {
		if (pdev->bus->number != (sd_pci_slot>>16) ||
			...
			SDLX_MSG(("%s: %s: bus %X, slot %X, vend %X, dev %X\n",
				__FUNCTION__,
				ifxsdh_chipmatch(pdev->vendor, pdev->device)
	...
	if (!ifxsdh_chipmatch(pdev->vendor, pdev->device)) {
	...
	/* use ifxsdh_query_device() to get the vendor ID of the target device so
 ...
 * module init
*/
int ifxsdh_register_client_driver(void)
{
	int error = 0;
 ...
 * module cleanup
*/
void ifxsdh_unregister_client_driver(void)
{
	sd_trace(("%s Enter\n", __FUNCTION__));
#if defined(BCMSPI) && defined(BCMPCISPIHOST)
