...
extern bcmsdh_info_t *ifxsdh_attach(osl_t *osh, void *sdioh, ulong *regsva);
/**
 * BCMSDH API context
 */
struct bcmsdh_info
...
extern int ifxsdh_detach(osl_t *osh, void *sdh);

/* Query if SD device interrupts are enabled */
extern bool ifxsdh_intr_query(void *sdh);

/* Enable/disable SD interrupt */
extern int ifxsdh_intr_enable(void *sdh);
extern int ifxsdh_intr_disable(void *sdh);

/* Register/deregister device interrupt handler. */
extern int ifxsdh_intr_reg(void *sdh, bcmsdh_cb_fn_t fn, void *argh);
extern int ifxsdh_intr_dereg(void *sdh);
/* Enable/disable SD card interrupt forward */
extern void bcmsdh_intr_forward(void *sdh, bool pass);

#if defined(DHD_DEBUG)
/* Query pending interrupt status from the host controller */
extern bool ifxsdh_intr_pending(void *sdh);
#endif // endif

/* Register a callback to be called if and when bcmsdh detects
 * device removal. No-op in the case of non-removable/hardwired devices.
 */
extern int ifxsdh_devremove_reg(void *sdh, bcmsdh_cb_fn_t fn, void *argh);
 ...
 */
extern uint8 ifxsdh_cfg_read(void *sdh, uint func, uint32 addr, int *err);
extern void ifxsdh_cfg_write(void *sdh, uint func, uint32 addr, uint8 data, int *err);

/* Read/Write 4bytes from/to cfg space */
extern uint32 ifxsdh_cfg_read_word(void *sdh, uint fnc_num, uint32 addr, int *err);
extern void ifxsdh_cfg_write_word(void *sdh, uint fnc_num, uint32 addr, uint32 data, int *err);
 ...
 */
extern int ifxsdh_cis_read(void *sdh, uint func, uint8 *cis, uint length);
 ...
 */
extern uint32 ifxsdh_reg_read(void *sdh, uintptr addr, uint size);
extern uint32 ifxsdh_reg_write(void *sdh, uintptr addr, uint size, uint32 data);

/* set sb address window */
extern int bcmsdhsdio_set_sbaddr_window(void *sdh, uint32 address, bool force_set);

/* Indicate if last reg read/write failed */
extern bool ifxsdh_regfail(void *sdh);
...
extern int ifxsdh_send_buf(void *sdh, uint32 addr, uint fn, uint flags,
                           uint8 *buf, uint nbytes, void *pkt,
                           bcmsdh_cmplt_fn_t complete_fn, void *handle);
extern int ifxsdh_recv_buf(void *sdh, uint32 addr, uint fn, uint flags,
                           uint8 *buf, uint nbytes, void *pkt,
                           bcmsdh_cmplt_fn_t complete_fn, void *handle);
 ...
 * Returns 0 or error code.
 */
extern int ifxsdh_rwdata(void *sdh, uint rw, uint32 addr, uint8 *buf, uint nbytes);

/* Issue an abort to the specified function */
extern int ifxsdh_abort(void *sdh, uint fn);
...
extern int ifxsdh_waitlockfree(void *sdh);

/* Returns the "Device ID" of target device on the SDIO bus. */
extern int ifxsdh_query_device(void *sdh);

/* Returns the number of IO functions reported by the device */
extern uint ifxsdh_query_iofnum(void *sdh);

/* Miscellaneous knob tweaker. */
extern int ifxsdh_iovar_op(void *sdh, const char *name,
                           void *params, uint plen, void *arg, uint len, bool set);

/* Reset and reinitialize the device */
extern int ifxsdh_reset(bcmsdh_info_t *sdh);
...
extern int ifxsdh_register(bcmsdh_driver_t *driver);
extern void ifxsdh_unregister(void);
extern bool ifxsdh_chipmatch(uint16 vendor, uint16 device);
...
extern uint32 ifxsdh_get_dstatus(void *sdh);

/* Function to return current window addr */
extern uint32 ifxsdh_cur_sbwad(void *sdh);
...
extern void ifxsdh_chipinfo(void *sdh, uint32 chip, uint32 chiprev);
