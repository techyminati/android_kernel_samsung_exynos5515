...
/* driver info, initialized when ifxsdh_register is called */
...
extern volatile bool dhd_is_mmc_wakeup;
 ...
 */
bool
ifxsdh_chipmatch(uint16 vendor, uint16 device)
{
	...
	uint bus_num, uint slot_num)
{
	...
	bcmsdh = ifxsdh_attach(osh, sdioh, &regs);
	if (bcmsdh == NULL) {
		SDLX_MSG(("%s: ifxsdh_attach failed\n", __FUNCTION__));
		goto err;
	}
	...
	vendevid = ifxsdh_query_device(bcmsdh);
...
int bcmsdh_remove(bcmsdh_info_t *bcmsdh)
{
	...
	ifxsdh_detach(bcmsdh->osh, bcmsdh);
...
extern int ifxsdh_register_client_driver(void);
extern void ifxsdh_unregister_client_driver(void);
...
int
ifxsdh_register(bcmsdh_driver_t *driver)
{
	int error = 0;

	drvinfo = *driver;
	SDLX_MSG(("%s: register client driver\n", __FUNCTION__));
	error = ifxsdh_register_client_driver();
	if (error)
		SDLX_MSG(("%s: failed %d\n", __FUNCTION__, error));
...
ifxsdh_unregister(void)
{
	...
	ifxsdh_unregister_client_driver();
}
...
#ifdef BCMSDH_MODULE
EXPORT_SYMBOL(ifxsdh_attach);
EXPORT_SYMBOL(ifxsdh_detach);
EXPORT_SYMBOL(ifxsdh_intr_query);
EXPORT_SYMBOL(ifxsdh_intr_enable);
EXPORT_SYMBOL(ifxsdh_intr_disable);
EXPORT_SYMBOL(ifxsdh_intr_reg);
EXPORT_SYMBOL(ifxsdh_intr_dereg);

#if defined(DHD_DEBUG)
EXPORT_SYMBOL(ifxsdh_intr_pending);
#endif // endif
...
EXPORT_SYMBOL(ifxsdh_devremove_reg);
EXPORT_SYMBOL(ifxsdh_cfg_read);
EXPORT_SYMBOL(ifxsdh_cfg_write);
EXPORT_SYMBOL(ifxsdh_cis_read);
EXPORT_SYMBOL(ifxsdh_reg_read);
EXPORT_SYMBOL(ifxsdh_reg_write);
EXPORT_SYMBOL(ifxsdh_regfail);
EXPORT_SYMBOL(ifxsdh_send_buf);
EXPORT_SYMBOL(ifxsdh_recv_buf);

EXPORT_SYMBOL(ifxsdh_rwdata);
EXPORT_SYMBOL(ifxsdh_abort);
EXPORT_SYMBOL(ifxsdh_query_device);
EXPORT_SYMBOL(ifxsdh_query_iofnum);
EXPORT_SYMBOL(ifxsdh_iovar_op);
EXPORT_SYMBOL(ifxsdh_register);
EXPORT_SYMBOL(ifxsdh_unregister);
EXPORT_SYMBOL(ifxsdh_chipmatch);
EXPORT_SYMBOL(ifxsdh_reset);
EXPORT_SYMBOL(ifxsdh_waitlockfree);

EXPORT_SYMBOL(ifxsdh_get_dstatus);
EXPORT_SYMBOL(ifxsdh_cfg_read_word);
EXPORT_SYMBOL(ifxsdh_cfg_write_word);
EXPORT_SYMBOL(ifxsdh_cur_sbwad);
EXPORT_SYMBOL(ifxsdh_chipinfo);
