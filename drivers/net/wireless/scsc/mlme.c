...
static struct slsi_mlme_parameters *slsi_mlme_append_channel_ie(struct slsi_mlme_parameters *ie,
								struct sk_buff *req)
{
	memset(ie, 0, sizeof(struct slsi_mlme_parameters));
					 ...
					 bool                               passive_scan)
{
	...
	channels_list_ie_head = slsi_mlme_append_channel_ie(&channels_list_ie, req);
	if (!channels_list_ie_head) {
		SLSI_WARN(sdev, "channel list IE append failed\n");
		kfree_skb(req);
		return -EINVAL;
	...
	for (i = 0; i < num_channels; i++) {
		...
		n_valid_channels++;
		channels_list_ie_head->length += SLSI_SCAN_CHANNEL_DESCRIPTOR_SIZE;
		if ((n_valid_channels % SLSI_MLME_MAX_CHAN_COUNT == 0) && (i + 1 != num_channels)) {
			channels_list_ie_head = slsi_mlme_append_channel_ie(&channels_list_ie, req);
			if (!channels_list_ie_head) {
				SLSI_WARN(sdev, "channel list IE append failed\n");
				kfree_skb(req);
