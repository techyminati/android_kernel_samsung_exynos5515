...
static struct sk_buff *hip5_bd_to_skb(struct scsc_service *service, struct hip_priv *hip_priv, struct hip5_hip_entry *hip_entry, scsc_mifram_ref *to_free, u16 *idx_r)
{
	struct slsi_hip         *hip;
...
static int hip5_q_add_bd_entry(struct slsi_hip *hip, enum hip5_hip_q_conf conf, scsc_mifram_ref phy_m, struct scsc_service *service, void *m_ptr, void *sig_ptr)
{
	...
	hip5_q_entry->data_len = m->len; 						/* total length of data in multiple HIP entries */
	...
	/* FAPI signal */
	memcpy(hip5_q_entry->fapi_sig + 4, sig_ptr, fapi_sig_size(ma_unitdata_req));
...
static int hip5_napi_poll(struct napi_struct *napi, int budget)
{
	...
	while (idx_r != idx_w) {
		...
		/* Catch-up with idx_w */
		hip_entry = (struct hip5_hip_entry *)(&ctrl->q_tlv[2].array[idx_r]);
		skb = hip5_bd_to_skb(service, hip_priv, hip_entry, to_free, &idx_r);
		if (!skb) {
...
int slsi_hip_init(struct slsi_hip *hip)
{
	...
	if (hip4_qos_enable) {
		if (!scsc_service_pm_qos_add_request(service, SCSC_QOS_DISABLED)) {
			...
			if (slsi_traffic_mon_client_register(sdev, hip, TRAFFIC_MON_CLIENT_MODE_EVENTS, (hip4_qos_med_tput_in_mbps * 1000 * 1000), (hip4_qos_max_tput_in_mbps * 1000 * 1000), hip5_traffic_monitor_cb))
	...
	if (slsi_traffic_mon_client_register(sdev, hip->hip_priv, TRAFFIC_MON_CLIENT_MODE_EVENTS, 0, (hip4_dynamic_logging_tput_in_mbps * 1000 * 1000), hip5_traffic_monitor_logring_cb))
