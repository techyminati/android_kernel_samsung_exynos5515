...
#error "To ENABLE NAN set CONFIG_SCSC_WIFI_NAN_ENABLE to y and CONFIG_SCSC_WLAN_MAX_INTERFACES >= 13"
#endif
#endif
...
struct slsi_dev *slsi_dev_attach(struct device *dev, struct scsc_mx *core, struct scsc_service_client *mx_wlan_client)
{
	...
	if (slsi_netif_register(sdev, sdev->netdev[SLSI_NET_INDEX_AP]) != 0) {
...
err_ap_registered:
	slsi_netif_remove(sdev, sdev->netdev[SLSI_NET_INDEX_AP]);
