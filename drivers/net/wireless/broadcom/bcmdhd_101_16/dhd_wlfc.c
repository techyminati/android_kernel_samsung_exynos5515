...
#if defined(SUPPORT_P2P_GO_PS) || defined(ONOFF_WLFC_EVENT_SDIO_FC)

static bool
_dhd_wlfc_suspend_resume_allowed(dhd_pub_t *dhd)
{
	/* Handle PS on/off indication */
	athost_wl_status_info_t* wlfc = (athost_wl_status_info_t*)dhd->wlfc_state;
	wlfc_mac_descriptor_t* table;
	...
	for (i = 0; i < WLFC_MAX_IFNUM; i++) {
		table = wlfc->destination_entries.interfaces;
		if (table[i].occupied) {
			if (table[i].state == WLFC_STATE_CLOSE) {
				return FALSE;
			}
		}
	}
...
dhd_wlfc_suspend(dhd_pub_t *dhd)
{
	...
	if (!_dhd_wlfc_suspend_resume_allowed(dhd)) {
		DHD_ERROR(("%s, interface close - tlv/host_suspend is not set\n", __FUNCTION__));
		goto end;
	}
	...
	ret = dhd_wl_ioctl_set_intiovar(dhd, "bus:host_suspend", 1, WLC_SET_VAR, TRUE, 0);
	if (unlikely(ret)) {
		DHD_ERROR(("%s, failed to set bus:host_suspend ret =%d\n", __FUNCTION__, ret));
		ret = BCME_ERROR;
		goto end;
	}
	...
	DHD_ERROR(("%s, tlv = 0x%08x, ret = %d\n", __FUNCTION__, tlv, ret));
	return ret;
}
...
dhd_wlfc_resume(dhd_pub_t *dhd)
{
	...
	if (!_dhd_wlfc_suspend_resume_allowed(dhd)) {
		DHD_ERROR(("%s, interface close\n", __FUNCTION__));
	}

#if defined(CONFIG_BCM43013)
	ret = dhd_wl_ioctl_set_intiovar(dhd, "bus:host_suspend", 0, WLC_SET_VAR, TRUE, 0);
	if (unlikely(ret)) {
		DHD_ERROR(("%s, failed to set bus:host_suspend ret =%d\n", __FUNCTION__, ret));
		ret = BCME_ERROR;
		goto end;
...
#endif /* SUPPORT_P2P_GO_PS || ONOFF_WLFC_EVENT_SDIO_FC */
