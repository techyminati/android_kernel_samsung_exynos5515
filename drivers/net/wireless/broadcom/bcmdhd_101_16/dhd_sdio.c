...
int
dhdsdio_bussleep(struct dhd_bus *bus, bool sleep)
{
	int err = 0;
	bcmsdh_info_t *sdh = bus->sdh;
...
extern volatile bool dhd_sdio_wakeup_pkt_log;
...
#endif
#ifdef ENABLE_WAKEUP_PKT_DUMP
			if (dhd_sdio_wakeup_pkt_log) {
				prhex("1. Rx Data", rxbuf, (len > 48 ? 48 : len));
				dhd_sdio_wakeup_pkt_log = FALSE;
			}
#endif /* ENABLE_WAKEUP_PKT_DUMP */
...
#ifdef ENABLE_WAKEUP_PKT_DUMP
		if (dhd_sdio_wakeup_pkt_log) {
			prhex("2. Rx Data", PKTDATA(osh, pkt), (len > 48 ? 48 : len));
			dhd_sdio_wakeup_pkt_log = FALSE;
		}
#endif /* ENABLE_WAKEUP_PKT_DUMP */
deliver:
