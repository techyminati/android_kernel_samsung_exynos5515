...
volatile bool dhd_sdio_wakeup_pkt_log = FALSE;
long long temp_raw;
...
#endif /* (SUPPORT_P2P_GO_PS || ONOFF_WLFC_EVENT_SDIO_FC) && PROP_TXSTATUS */

#if defined(BCMSDIO)
	if (suspend) {
		int rc = dhdsdio_bussleep(dhdinfo->pub.bus, TRUE);
		if (rc != BCME_OK) {
			DHD_ERROR(("%s: bus sleep err(%d)\n", __FUNCTION__, rc));
		}
	}
#endif /* BCMSDIO */
...
_dhd_module_init(void)
{
	...
	if (!root_node) {
		DHD_ERROR(("failed to get device node of bcm-wlan\n"));
		return -ENODEV;
 	}
