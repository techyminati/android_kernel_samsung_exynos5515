...
static int dw_mci_warranty_bit;
...
static int __init dw_mci_get_warranty_bit(char *str)
{
	dw_mci_warranty_bit = memparse(str, &str);

	return 0;
}
__setup("androidboot.warranty_bit=", dw_mci_get_warranty_bit);
...
static void dw_mci_mmc_error_count_log(struct dw_mci *host, int index, int error)
{
...
void dw_mci_mmc_error_logging(struct dw_mci *host, u32 status, int err, u32 type, bool forced)
{
	...
	/*
	 * Don't error log for these cases except forced flag is set.
	 * 1. eMMC/SD initialize isn't completed yet.
	 * 2. eMMC/SD enter suspend.
	 * 3. eMMC/SD is in error recovery.
	 * 4. SDIO card.
	 */
	if (!forced && (!card || mmc_card_suspended(card) || mmc_card_recovery(card) || mmc_card_sdio(card)))
		return;

	err_log = host->err_log;

	if (status & STATUS_ERR_MASK) {
		if ((status & HALT_UNHALT_ERR) && err_log[0].halt_cnt < MAX_CNT_U32)
			err_log[0].halt_cnt++;
		if ((status & CQ_EN_DIS_ERR) && err_log[0].cq_cnt < MAX_CNT_U32)
			err_log[0].cq_cnt++;
		if ((status & RPMB_SWITCH_ERR) && err_log[0].rpmb_cnt < MAX_CNT_U32)
			err_log[0].rpmb_cnt++;
		if ((status & HW_RST) && err_log[0].hw_rst_cnt < MAX_CNT_U32) {
		...
		if ((status & R1_ERROR) && err_log[0].ge_cnt < MAX_CNT_U32)
			err_log[0].ge_cnt++;
		if ((status & R1_CC_ERROR) && err_log[0].cc_cnt < MAX_CNT_U32)
			err_log[0].cc_cnt++;
		if ((status & R1_CARD_ECC_FAILED) && err_log[0].ecc_cnt < MAX_CNT_U32)
			err_log[0].ecc_cnt++;
		if ((status & R1_WP_VIOLATION) && err_log[0].wp_cnt < MAX_CNT_U32)
			err_log[0].wp_cnt++;
		if ((status & R1_OUT_OF_RANGE) && err_log[0].oor_cnt < MAX_CNT_U32)
			err_log[0].oor_cnt++;
	}

	if (type == CMD_ERROR_LOG || type == SW_CMD_TIMEOUT_LOG) {
		if (err == -EILSEQ || err == -EIO)
			dw_mci_mmc_error_count_log(host, 0, err);
		if (err == -ETIMEDOUT)
			dw_mci_mmc_error_count_log(host, 1, err);
		if (err == -ENOMEDIUM)
			dw_mci_mmc_error_count_log(host, 2, err);
	} else if (type == DATA_ERROR_LOG || type == SW_DATA_TIMEOUT_LOG) {
		if (err == -EILSEQ)
			dw_mci_mmc_error_count_log(host, 3, err);
		if (err == -ETIMEDOUT)
			dw_mci_mmc_error_count_log(host, 4, err);
		if (err == -ENOMEDIUM)
			dw_mci_mmc_error_count_log(host, 5, err);
	}
}

void mmc_card_debug_log_sysfs_init(struct dw_mci *host)
{
	if (!host) {
		pr_info("%s : Failed to init err_log.\n", __func__);
		return;
	}

	/* init. card->err_log */
	snprintf(host->err_log[0].type, sizeof(char)*5, "cmd  ");
	snprintf(host->err_log[1].type, sizeof(char)*5, "cmd  ");
	snprintf(host->err_log[2].type, sizeof(char)*5, "cmd  ");
	snprintf(host->err_log[3].type, sizeof(char)*5, "data ");
	snprintf(host->err_log[4].type, sizeof(char)*5, "data ");
	snprintf(host->err_log[5].type, sizeof(char)*5, "data ");

	host->err_log[0].err_type = -EILSEQ;
	host->err_log[1].err_type = -ETIMEDOUT;
	host->err_log[2].err_type = -ENOMEDIUM;
	host->err_log[3].err_type = -EILSEQ;
	host->err_log[4].err_type = -ETIMEDOUT;
	host->err_log[5].err_type = -ENOMEDIUM;
}
...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		...
		if (mmc->card && mmc_card_recovery(mmc->card))
			dw_mci_mmc_error_logging(host, HW_RST, 0, 0, 1);
...
static void dw_mci_get_error_log(struct mmc_host *mmc)
{
	struct dw_mci_slot *slot = mmc_priv(mmc);
	struct dw_mci *host = slot->host;
	int i;
	...
	for (i = 0; i < 6; i++) {
		...
		if ((err_log[i].err_type == -ETIMEDOUT || err_log[i].err_type == -ENOMEDIUM)
			&& total_t_cnt < MAX_CNT_U64)
			total_t_cnt += err_log[i].count;
	}

	dev_info(host->dev, "GE:%d,CC:%d,ECC:%d,WP:%d,OOR:%d,CRC:%lld,TMO:%lld,HALT:%d,CQEN:%d,RPMB:%d,RST:%d\n",
...
static const struct mmc_host_ops dw_mci_ops = {
	...
	.get_error_log		= dw_mci_get_error_log,
};
       ...
       case MMC_POWER_ON:
		...
		if (pdata->pinctrl && pdata->clk_drive_base)
			pinctrl_select_state(pdata->pinctrl, pdata->clk_drive_base);
               ...
               break;
       case MMC_POWER_OFF:
               // If warranty bit is set, skip eMMC power off.
               if (dw_mci_warranty_bit == 1) {
                       dev_info(host->dev, "%s : Warranty_bit is set. Skip mmc turn off", __func__);
                       break;
               }
               ...
               msleep(pdata->emmc_pwr.dis_charge);     /* Discharging Time */
               break;
       }
...
static int dw_mci_command_complete(struct dw_mci *host, struct mmc_command *cmd)
{
	...
	if (cmd->flags & MMC_RSP_PRESENT) {
		if (cmd->flags & MMC_RSP_136) {
			...
			if (STATUS_ERR_MASK & cmd->resp[0])
				dw_mci_mmc_error_logging(host, cmd->resp[0], 0, 0, 0);
		}
	}
	...
	if (cmd->error)
		dw_mci_mmc_error_logging(host, 0, cmd->error, CMD_ERROR_LOG, 0);
...
static int dw_mci_data_complete(struct dw_mci *host, struct mmc_data *data)
{
	...
	if (status & DW_MCI_DATA_ERROR_FLAGS) {
		...
		dw_mci_mmc_error_logging(host, 0, data->error, DATA_ERROR_LOG, 0);
...
static irqreturn_t dw_mci_interrupt(int irq, void *dev_id)
{
	...
	if (host->slot->mmc->card &&
		...
		if (reg) {
			...
			if (cmd_error)
				dw_mci_mmc_error_logging(host, 0, cmd_error, CMD_ERROR_LOG, 0);
			...
			if (data_error)
				dw_mci_mmc_error_logging(host, 0, data_error, DATA_ERROR_LOG, 0);
		} else {
			cmd_error = 0;
			data_error = 0;
		}
		...
		if (cmd_error || data_error) {
			if (host->cmd && !(host->cmd->flags & MMC_RSP_136)) {
				status = mci_readl(host, RESP0);
				if (STATUS_ERR_MASK & status)
					dw_mci_mmc_error_logging(host, status, 0, 0, 0);
			}
		}
...
static void dw_mci_timeout_timer(struct timer_list *t)
{
	...
	if (host && host->mrq) {
		...
		case STATE_SENDING_CMD:
			mrq->cmd->error = -ENOMEDIUM;
			dw_mci_mmc_error_logging(host, 0, -ENOMEDIUM, SW_CMD_TIMEOUT_LOG, 0);
			if (!mrq->data)
				break;
			/* fall through */
		case STATE_SENDING_DATA:
			mrq->data->error = -ENOMEDIUM;
			dw_mci_mmc_error_logging(host, 0, -ENOMEDIUM, SW_DATA_TIMEOUT_LOG, 0);
			dw_mci_stop_dma(host);
			break;
		case STATE_DATA_BUSY:
		case STATE_DATA_ERROR:
...
int dw_mci_probe(struct dw_mci *host, struct platform_device *pdev)
{
	...
	mmc_card_debug_log_sysfs_init(host);
	return 0;
